...
% working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2018\14Jun2019\";
...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/14Jun2019");
...
%% Gather the DISY_14Jun2019 Data from the Forerunner Matrix 'PD_mtrx'
...
% the 0's will indicate rows that are to be removed.
    % For the DISY_14Jun2019, the data presented groups of four thus, only
...
% Reassign the variables so that they represent the filtered information
    ...
    smry_DISY_14Jun2019 = summary(TT_DISY);
    % Clear extraneous variables
...
subplot(1,2,1)
    plot(TT_DISY.Time, TT_DISY.HP_CH4_dry, 'o')
        title('DISY 14Jun2019 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_DISY.Time, TT_DISY.CO2_dry, 'mo')
        title('DISY 14Jun2019 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
% --- CH4 --- %
   h1 = plot(TT_DISY.Time,TT_DISY.HP_CH4_dry, 'o');
                    title('DISY 14Jun2019 [CH_4] and [CO_2] vs. Time')
                    grid on
