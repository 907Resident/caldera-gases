...
% Metadata File: "...\Yellowstone\Jul2018\03Jul2018\WTHB_03Jul2018\WTHB_03Jul2018_Metadata"
...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/03Jul2018/WTHB_03Jul2018");
...
%% Gather the WTHB_03Jul2018 Data from the Forerunner Matrix 'PD_mtrx'
...
% the 0's will indicate rows that are to be removed.
    % For the WTHB_03Jul2018, the data presented groups of four thus, only
...
% Reassign the variables so that they represent the filtered information
    ...
    smry_WTHB_03Jul2018 = summary(TT_WTHB);
    % Clear extraneous variables
...
subplot(1,2,1)
    plot(TT_WTHB.Time, TT_WTHB.HP_CH4_dry, 'o')
        title('WTHB 03Jul2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_WTHB.Time, TT_WTHB.CO2_dry, 'mo')
        title('WTHB 03Jul2018 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
% --- CH4 --- %
   h1 = plot(TT_WTHB.Time,TT_WTHB.HP_CH4_dry, 'o');
                    title('WTHB 03Jul2018 [CH_4] and [CO_2] vs. Time')
                    grid on
