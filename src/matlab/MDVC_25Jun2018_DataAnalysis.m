...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/25Jun2018");
...
%% Gather the MDVC_25Jun2018 Data from the Forerunner Matrix 'PD_mtrx'
...
% Reassign the variables so that they represent the filtered information
    ...
    smry_MDVC_25Jun2018 = summary(TT_MDVC);
    % Clear extraneous variables
...
%% Extract the individual measurement
    ...
    MDVC_PreBack    = NaN([115, width(TT_MDVC)+1, nchams]); 
    MDVC_ChamON     = NaN([400, width(TT_MDVC)+1, nchams]);
    MDVC_PostBack   = NaN([115, width(TT_MDVC)+1, nchams]);
...
for idx = 1:nchams
    ...
    % ChamON
        % Pre-allocate
    MDVC_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
...
MDVC_ChamON = gas_unit_prep(MDVC_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
if writeXL == 1
    ...
    for i = 1:nchams
        ...
        msg_str = sprintf('Tab ''MDVC_%d'' has been created', i);
        disp(msg_str)
    end
