...
%% CSTL_02Jul2018_DataAnalysis

% CSTL_02Jul2018_DataAnalysis
% Geoscientists: Ajayi, M. & Mans, W.
% Location: Castle Geyser, Yellowstone National Park, WY 
% [44.4641, -110.8361]
% Data Type: Transect Measurements 
% Metadata File: "...\Yellowstone\Jul2018\02Jul2018\CSTL_02Jul2018\CSTL_02Jul2018_Metadata"
...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/02Jul2018/CSTL_02Jul2018");
...
%% Gather the CSTL_02Jul2018 Data from the Forerunner Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData    = addvars(TT_PicData,CSTL_rel_sec,                    ...
                           'Before','HP_CH4_dry');
...
% the 0's will indicate rows that are to be removed.
    % For the CSTL_02Jul2018, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), CSTL_rel_sec                    ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_CSTL*
    TT_CSTL       = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),        ...
    ...
    smry_CSTL_02Jul2018 = summary(TT_CSTL);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_CSTL.Time, TT_CSTL.HP_CH4_dry, 'o')
        title('CSTL 02Jul2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_CSTL.Time, TT_CSTL.CO2_dry, 'mo')
        title('CSTL 02Jul2018 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_CSTL.Time,TT_CSTL.HP_CH4_dry, 'o');
                    title('CSTL 02Jul2018 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_CSTL.Time,TT_CSTL.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
%% Extract the individual measurement
    ...
    CSTL_PreBack    = NaN([115, width(TT_CSTL)+1, nchams]); 
    CSTL_ChamON     = NaN([400, width(TT_CSTL)+1, nchams]);
    CSTL_PostBack   = NaN([115, width(TT_CSTL)+1, nchams]);
...
%% Extract  datetimes
    ...
    % PreBack
        % Pre-allocate
    CSTL_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(CSTL_PreBack(:,1,idx)));
    CSTL_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(CSTL_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    CSTL_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(CSTL_ChamON(:,1,idx)));
    CSTL_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(CSTL_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    CSTL_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(CSTL_PostBack(:,1,idx)));
    CSTL_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(CSTL_PostBack(1:min(f)-1,1,idx)));
end
...
[fig03, fig04, fig05] = basic_gas_viz(CSTL_ChamON, nchams, site_tag,    ...
...
% c_CH4_ppm                       = CSTL_ChamON(:,3,:);   
% c_CO2_ppm                       = CSTL_ChamON(:,7,:);
% % Chameber Height (m)
%     % 5in = 12.7 cm = 0.127 m
% H                               = 0.127; 
...
%  for idx = 1:nchams
%         CSTL_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
%         CSTL_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
%  end
% 
% % Clear extraneous variables
...
CSTL_ChamON = gas_unit_prep(CSTL_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(CSTL_ChamON(:,02,i),                  ...
                                  CSTL_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ CSTL_ChamON(:,3,idx)),           ...
                                         CSTL_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ CSTL_ChamON(:,7,idx)),         ...
                                         CSTL_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(CSTL_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(CSTL_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = CSTL_ChamON(:,5,:); iCO2 = CSTL_ChamON(:,8,:);
...
CSTL_ChamON_Ratio           = CSTL_ChamON(:,7,:) ./ CSTL_ChamON(:,3,:);
...
CSTL_ChamON_Ratio_stats     = [mean(CSTL_ChamON_Ratio);                 ...
                               median(CSTL_ChamON_Ratio);               ...
                               range(CSTL_ChamON_Ratio);                ...
                               std(CSTL_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(CSTL_ChamON(:,2,idx), CSTL_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = CSTL_ChamON(:,5,i); Y = CSTL_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = CSTL_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(CSTL_ChamON_Ratio) max(CSTL_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(CSTL_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = CSTL_ChamON(:,5,i);
        xyz(:,2,i) = CSTL_ChamON(:,8,i);
        xyz(:,3,i) = CSTL_ChamON_Ratio(:,1,i);
    end
...
CSTL_PreBack(CSTL_PreBack == 0)     = NaN;
CSTL_PostBack(CSTL_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(CSTL_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(CSTL_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(CSTL_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(CSTL_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(CSTL_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(CSTL_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(CSTL_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(CSTL_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(CSTL_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    if all(isnan(CSTL_PostBack(:,3,i)))
        continue
    end

    [f, xi, bw_post] = ksdensity(CSTL_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(CSTL_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(CSTL_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(CSTL_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(CSTL_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(CSTL_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(CSTL_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    ...
    for i = 1:nchams
        ...
        export_ChamON_times     = string(CSTL_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = CSTL_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(CSTL_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(CSTL_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = CSTL_PreBack (:,3,i);
        export_PreB_CO2         = CSTL_PreBack (:,7,i);
        export_PreB_iCH4        = CSTL_PreBack (:,5,i);
        export_PreB_iCO2        = CSTL_PreBack (:,8,i);
        export_PostB_CH4        = CSTL_PostBack(:,3,i);
        export_PostB_CO2        = CSTL_PostBack(:,7,i);
        export_PostB_iCH4       = CSTL_PostBack(:,5,i);
        export_PostB_iCO2       = CSTL_PostBack(:,8,i);
        ...
        msg_str = sprintf('Tab ''CSTL_%d'' has been created', i);
        disp(msg_str)
    end
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            'CSTL_PreBack_DateTimes' , 'CSTL_PreBack' , ...
                            'CSTL_ChamON_DateTimes'  , 'CSTL_ChamON'  , ...
                            'CSTL_PostBack_DateTimes', 'CSTL_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
