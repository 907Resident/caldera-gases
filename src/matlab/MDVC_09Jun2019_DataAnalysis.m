...
%% MDVC_09Jun2019_DataAnalysis

% MDVC_09Jun2019_DataAnalysis
% Geoscientists: Ajayi, M. & Ayers, J.
% Location: Mud Volcano, Yellowstone National Park, WY 
% [44.623161, -110.435648]
% Data Type: Transect Measurements 
% Metadata File: "...\Yellowstone\Jun2019\09Jun2019\MDVC_09Jun2019\MDVC_09Jun2019_Metadata"
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2019\09Jun2019\";
...
%% Gather the MDVC_09Jun2019 Data from the Eosense Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData          = addvars(TT_PicData,MDVC_rel_sec,              ...
                         'Before','HP_CH4_dry');
...
weather_file = "E:\moyoa\Documents\OneDrive - Vanderbilt\Python\Yellowstone-Python\MDVC\MDVC_09Jun2019_Weather.csv";
Weather_Data = Import_Weather(weather_file);

% Convert temperatures from Fahrenheit to Celsius
Weather_Data.DewPoint   = (Weather_Data.DewPoint - 32) * (5/9);
Weather_Data.AmbientTemp= (Weather_Data.AmbientTemp - 32) * (5/9);
...
% the 0's will indicate rows that are to be removed.
    % For the MDVC_09Jun2019, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), MDVC_rel_sec          ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_MDVC_09Jun2019*
    TT_MDVC = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),    ...
    ...
    smry_MDVC_09Jun2019 = summary(TT_MDVC);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_MDVC.Time, TT_MDVC.HP_CH4_dry, 'o')
        title('MDVC 09Jun2019 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
...
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_MDVC.Time, TT_MDVC.CO2_dry, 'mo')
        title('MDVC 09Jun2019 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
fig_file = working_dir+"MATLAB_figs\MDVC_09Jun2019_CH4_and_CO2_timeline_side-by-side.fig";
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_MDVC.Time,TT_MDVC.HP_CH4_dry, 'o');
                    title('MDVC 09Jun2019 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_MDVC.Time,TT_MDVC.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
% Save figure as .fig to working directory
fig_file = working_dir+"MATLAB_figs\MDVC_09Jun2019_CH4_and_CO2_timeline_overlay.fig";
...
['13:37:06'; '14:34:14'; '15:34:15'; '16:59:44'],                       ...
...
['13:42:19'; '14:39:51'; '15:40:13'; '17:05:42'],                       ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['13:42:23'; '14:39:54'; '15:40:16'; '17:11:09'],                       ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['13:49:17'; '14:54:35'; '15:43:58'; '17:25:47'],                       ...
 'InputFormat', 'HH:mm:ss');
...
%% Extract the Individual Measurement
    ...
    MDVC_PreBack    = NaN([100, width(TT_MDVC)+1, nchams]);
    MDVC_ChamON     = NaN([420, width(TT_MDVC)+1, nchams]);
    MDVC_PostBack   = NaN([100, width(TT_MDVC)+1, nchams]);
...
%% Extract Datetimes
    ...
    % PreBack
        % Pre-allocate
    MDVC_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(MDVC_PreBack(:,1,idx)));
    MDVC_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(MDVC_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    MDVC_ChamON_DateTimes           = NaT([350 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(MDVC_ChamON(:,1,idx)));
    MDVC_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(MDVC_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    MDVC_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(MDVC_PostBack(:,1,idx)));
    MDVC_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(MDVC_PostBack(1:min(f)-1,1,idx)));
end
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(MDVC_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        MDVC_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        MDVC_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(MDVC_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        MDVC_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
[fig03, fig04, fig05] = basic_gas_viz(MDVC_ChamON, nchams, site_tag,    ...
...
%% Convert gas measurements from ppm to volumetric density units (M L-3)

% Gather the start times of each measurements by collecting the first
% datetime of each ChamON column
srt_ChamONs = MDVC_ChamON_DateTimes(1,1,:);
    % Reshape to a size(MDVC_ChamON_DateTimes,3) x 1 vector
    srt_ChamONs =  reshape(srt_ChamONs,                                 ...
                          [size(MDVC_ChamON_DateTimes,3), 1]);
% Intialize the Temp and Press vectors
Temp = nan(size(srt_ChamONs));
Press= nan(size(srt_ChamONs));
Vm   = nan(size(srt_ChamONs));
% Execute for-lop to load in the temperature and pressure data
    % Define the loop to go through the number of start ChamON times
for i = 1:size(MDVC_ChamON_DateTimes,3)
    % Define the nested loop to go through each pair of times within the
    % Weather_Data table
    for j = 2:height(Weather_Data)
        % Use if-conditional to find if the ChamON start time lies between
        % the pair of Weather_Data times
        if isbetween(srt_ChamONs(i),                                    ...
                     Weather_Data.DATETIME(j-1), Weather_Data.DATETIME(j)) ...
                     == 1
           % If the conditional is successful, round the time to the
           % nearest hour
                % To do this, convert datetime to a datenum object
           ro_dte = datenum_round_off(datenum(MDVC_ChamON_DateTimes(i)),...
                                     'hour');
                % Convert rounded value back to datetime object
           ro_dte = datetime(datestr(ro_dte));
           % Use if-coonditional to establish which of the pair of
           % Weather_Data entries to use
                % Rounded down
           if ro_dte == Weather_Data.DATETIME(j-1) == 1
              Temp(i) = Weather_Data.AmbientTemp(j-1);
              Press(i)= Weather_Data.Pressure(j-1);
              continue
                % Rounded up
           else
              Temp(i) = Weather_Data.AmbientTemp(j);
              Press(i)= Weather_Data.Pressure(j);
              continue
           end         
        end
    end
% Gather the molar volumes (L m-3)
[~,Vm(i)]                       = MolVol(Temp(i),Press(i),'C','mbar');
end
% Use Repmat to make the dimensions of Vm match the measured values
Vm = reshape(Vm,[1,length(Vm)]);
Vm = repmat(Vm,[length(MDVC_ChamON), 1]);
...
c_CH4_ppm                       = MDVC_ChamON(:,3,:);   
c_CO2_ppm                       = MDVC_ChamON(:,7,:);
% Chamber Height (m)
...
%%% meter ug/m3
    ...
    %%% m^3)
        %%% (Molec_wt / Vm) * (1000 L / m^3) * ppm = mg / m^3
        c_CH4_mg_m3             = ((CH4_molec_wt ./ Vm)                 ...
                                    .* c_CH4_ppm);
    %%% Repeat for carbon dioxide     
        %%% (Molec_wt / Vm) * (1000 L / m^3) * ppm = mg / m^3
        c_CO2_mg_m3             = ((CO2_molec_wt ./ Vm)                 ...
                                    .* c_CO2_ppm);     
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        MDVC_ChamON(:,10,idx)   = c_CH4_mg_m3(:,1,idx);
        MDVC_ChamON(:,11,idx)   = c_CO2_mg_m3(:,1,idx);
 end

% Clear extraneous variables
clearvars N CH4_molec_wt CO2_molec_wt c_CH4_cgs_UnEx 
clearvars c_CO2_cgs_UnEx c_CH4_ppm c_CO2_ppm 

%% Flux Calculation

[lin_flux, lin_mdl, lin_slope, flx_srt_time, flx_end_time, fig06, fig07] = ...
 lin_flux_AC(MDVC_ChamON, "y");
% Save figure as .fig to working directory
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(MDVC_ChamON(:,02,i),                  ...
                                  MDVC_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ MDVC_ChamON(:,3,idx)),         ...
                                         MDVC_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ MDVC_ChamON(:,7,idx)),         ...
                                         MDVC_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(MDVC_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(MDVC_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = MDVC_ChamON(:,5,:); iCO2 = MDVC_ChamON(:,8,:);
...
MDVC_ChamON_Ratio           = MDVC_ChamON(:,7,:) ./ MDVC_ChamON(:,3,:);
...
MDVC_ChamON_Ratio_stats     = [mean(MDVC_ChamON_Ratio);                 ...
                               median(MDVC_ChamON_Ratio);               ...
                               range(MDVC_ChamON_Ratio);                ...
                               std(MDVC_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(MDVC_ChamON(:,2,idx), MDVC_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = MDVC_ChamON(:,5,i); Y = MDVC_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = MDVC_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(MDVC_ChamON_Ratio) max(MDVC_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(MDVC_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = MDVC_ChamON(:,5,i);
        xyz(:,2,i) = MDVC_ChamON(:,8,i);
        xyz(:,3,i) = MDVC_ChamON_Ratio(:,1,i);
    end
...
MDVC_PreBack(MDVC_PreBack == 0)     = NaN;
MDVC_PostBack(MDVC_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(MDVC_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(MDVC_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(MDVC_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(MDVC_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(MDVC_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(MDVC_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(MDVC_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(MDVC_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(MDVC_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(MDVC_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(MDVC_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(MDVC_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(MDVC_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(MDVC_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(MDVC_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(MDVC_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    for i = 1:nchams
        ...
        export_ChamON_times     = string(MDVC_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = MDVC_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(MDVC_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(MDVC_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = MDVC_PreBack (:,3,i);
        export_PreB_CO2         = MDVC_PreBack (:,7,i);
        export_PreB_iCH4        = MDVC_PreBack (:,5,i);
        export_PreB_iCO2        = MDVC_PreBack (:,8,i);
        export_PostB_CH4        = MDVC_PostBack(:,3,i);
        export_PostB_CO2        = MDVC_PostBack(:,7,i);
        export_PostB_iCH4       = MDVC_PostBack(:,5,i);
        export_PostB_iCO2       = MDVC_PostBack(:,8,i);
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            'MDVC_PreBack_DateTimes' , 'MDVC_PreBack' , ...
                            'MDVC_ChamON_DateTimes'  , 'MDVC_ChamON'  , ...
                            'MDVC_PostBack_DateTimes', 'MDVC_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
