...
%% NEER_26Jun2018_Data Analysis

% NEER_26Jun2018_Data Analysis Geoscientists: Ajayi, M. & Mans, Wade.
% Location: Northeast Entrance, Yellowstone, WY (Northeastern Entrance =
% NEER)
% Data Type: Transect 01 | Pts. 01-04 (measured on 26 Jun 2018)
%            Transect 02 | Pts. 05-08 (measured on 26 Jun 2018)
% Metadata File: [To be added]
...
% working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2018\26Jun2018\";
...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/26Jun2018");
...
%% Gather the NEER_26Jun2018 Data from the Forerunner Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData    = addvars(TT_PicData,NEER_rel_sec,                    ...
                           'Before','HP_CH4_dry');
...
% the 0's will indicate rows that are to be removed.
    % For the NEER_26Jun2018, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), NEER_rel_sec                    ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_NEER*
    TT_NEER       = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),        ...
    ...
    smry_NEER_26Jun2018 = summary(TT_NEER);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_NEER.Time, TT_NEER.HP_CH4_dry, 'o')
        title('NEER 26Jun2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_NEER.Time, TT_NEER.CO2_dry, 'mo')
        title('NEER 26Jun2018 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_NEER.Time,TT_NEER.HP_CH4_dry, 'o');
                    title('NEER 26Jun2018 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_NEER.Time,TT_NEER.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
%----PreBack----% *****REDO the timestamps in this section********
    % Pre-allocate the datetime array
PreBack_Times                   = NaT([nchams, 2]);

PreBack_Times(:,1)              = datetime(                             ...
['11:06:52'; '11:45:31'; '12:29:17'; '13:09:46'; '14:00:01'; '14:40:49';...
 '15:23:03'; '16:00:44'],                                               ...
...
['11:11:53'; '11:50:32'; '12:33:44'; '13:14:19'; '14:05:02'; '14:45:41';...
 '15:28:10'; '16:05:40'],                                               ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['11:11:57'; '11:50:35'; '12:33:47'; '13:14:23'; '14:05:06'; '14:45:45';...
 '15:28:13'; '16:05:43'],                                               ...
 'InputFormat', 'HH:mm:ss');
ChamON_Times(:,1)               = DATE + timeofday(ChamON_Times(:,1));

ChamON_Times(:,2)               = datetime(                             ...
['11:33:08'; '12:11:50'; '12:54:40'; '13:34:57'; '14:27:06'; '15:05:29';...
 '15:49:13'; '16:27:07'],                                               ...
 'InputFormat', 'HH:mm:ss');
...
%----PostBack----%
% --- It is likely thta post-bakground data will be counted as
% pre-background data for the next chamber enclosure in the sequence ---- %
PostBack_Times                  = NaT([nchams, 2]);

PostBack_Times(:,1)             = datetime(                             ...
['11:34:15'; '12:12:32'; '12:55:23'; '13:35:43'; '14:28:06'; '15:05:32';...
 '15:50:06'; '16:28:10'],                                               ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['11:38:56'; '12:16:05'; '12:59:56'; '13:39:59'; '14:32:45'; '15:10:04';...
 '15:55:05'; '16:31:00'],                                               ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
% Extract the individual measurement
    ...
    NEER_PreBack    = NaN([100, width(TT_NEER)+1, nchams]);
    NEER_ChamON     = NaN([375, width(TT_NEER)+1, nchams]);
    NEER_PostBack   = NaN([100, width(TT_NEER)+1, nchams]);
...
%% Extract the individual measurement
    ...
    NEER_PreBack    = NaN([115, width(TT_NEER)+1, nchams]); 
    NEER_ChamON     = NaN([400, width(TT_NEER)+1, nchams]);
    NEER_PostBack   = NaN([115, width(TT_NEER)+1, nchams]);
...
%% Extract  datetimes
    ...
    % PreBack
        % Pre-allocate
    NEER_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(NEER_PreBack(:,1,idx)));
    NEER_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(NEER_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    NEER_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(NEER_ChamON(:,1,idx)));
    NEER_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(NEER_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    NEER_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(NEER_PostBack(:,1,idx)));
    NEER_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(NEER_PostBack(1:min(f)-1,1,idx)));
end
...
[fig03, fig04, fig05] = basic_gas_viz(NEER_ChamON, nchams, site_tag,    ...
...
% c_CH4_ppm                       = NEER_ChamON(:,3,:);   
% c_CO2_ppm                       = NEER_ChamON(:,7,:);
% % Chameber Height (m)
%     % 5in = 12.7 cm = 0.127 m
% H                               = 0.127; 
...
%  for idx = 1:nchams
%         NEER_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
%         NEER_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
%  end
% 
% % Clear extraneous variables
...
NEER_ChamON = gas_unit_prep(NEER_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(NEER_ChamON(:,02,i),                  ...
                                  NEER_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ NEER_ChamON(:,3,idx)),           ...
                                         NEER_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ NEER_ChamON(:,7,idx)),         ...
                                         NEER_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(NEER_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(NEER_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = NEER_ChamON(:,5,:); iCO2 = NEER_ChamON(:,8,:);
...
NEER_ChamON_Ratio           = NEER_ChamON(:,7,:) ./ NEER_ChamON(:,3,:);
...
NEER_ChamON_Ratio_stats     = [mean(NEER_ChamON_Ratio);                 ...
                               median(NEER_ChamON_Ratio);               ...
                               range(NEER_ChamON_Ratio);                ...
                               std(NEER_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(NEER_ChamON(:,2,idx), NEER_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = NEER_ChamON(:,5,i); Y = NEER_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = NEER_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(NEER_ChamON_Ratio) max(NEER_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(NEER_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = NEER_ChamON(:,5,i);
        xyz(:,2,i) = NEER_ChamON(:,8,i);
        xyz(:,3,i) = NEER_ChamON_Ratio(:,1,i);
    end
...
NEER_PreBack(NEER_PreBack == 0)     = NaN;
NEER_PostBack(NEER_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(NEER_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(NEER_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(NEER_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(NEER_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(NEER_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(NEER_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(NEER_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(NEER_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(NEER_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(NEER_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(NEER_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(NEER_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(NEER_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(NEER_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(NEER_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(NEER_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    ...
    for i = 1:nchams
        ...
        export_ChamON_times     = string(NEER_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = NEER_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(NEER_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(NEER_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = NEER_PreBack (:,3,i);
        export_PreB_CO2         = NEER_PreBack (:,7,i);
        export_PreB_iCH4        = NEER_PreBack (:,5,i);
        export_PreB_iCO2        = NEER_PreBack (:,8,i);
        export_PostB_CH4        = NEER_PostBack(:,3,i);
        export_PostB_CO2        = NEER_PostBack(:,7,i);
        export_PostB_iCH4       = NEER_PostBack(:,5,i);
        export_PostB_iCO2       = NEER_PostBack(:,8,i);
        ...
        msg_str = sprintf('Tab ''NEER_%d'' has been created', i);
        disp(msg_str)
    end
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            'NEER_PreBack_DateTimes' , 'NEER_PreBack' , ...
                            'NEER_ChamON_DateTimes'  , 'NEER_ChamON'  , ...
                            'NEER_PostBack_DateTimes', 'NEER_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
