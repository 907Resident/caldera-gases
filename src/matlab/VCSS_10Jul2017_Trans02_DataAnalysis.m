...
% Springs = VCSS)
% Data Type: Transect 02 | Pts. 01-03 (measured on 08 and 10 July 2017)
...
npts = input('How many cycles of closed-open chamber measurements took place?: ');
            
%% Assign vectors for the time intervals for 'DateTime'

% Pre-background measurements
    % Pre-Background
        VCSS_PreBack_DT     = datetime(['10-Jul-2017 13:01:01';           
                                        '10-Jul-2017 13:31:30';                      
                                        '10-Jul-2017 13:57:32';
                                        '10-Jul-2017 13:06:35';
                                        '10-Jul-2017 13:36:09';
                                        '10-Jul-2017 14:02:50']);
       VCSS_PreBack_DT      = reshape(VCSS_PreBack_DT, [npts 2]);
       
    % Chamber ON
        VCSS_ChamON_DT      = datetime(['10-Jul-2017 13:06:39';
                                        '10-Jul-2017 13:36:13';
                                        '10-Jul-2017 14:02:54';
                                        '10-Jul-2017 13:10:49';
                                        '10-Jul-2017 13:42:49';
                                        '10-Jul-2017 14:09:06']);
        VCSS_ChamON_DT      = reshape(VCSS_ChamON_DT, [npts 2]);
        
    % Post-Background
        VCSS_PostBack_DT    = datetime(['10-Jul-2017 13:12:24';
                                        '10-Jul-2017 13:44:06';
                                        '10-Jul-2017 14:10:34';
                                        '10-Jul-2017 13:17:39';
                                        '10-Jul-2017 13:49:56';
                                        '10-Jul-2017 14:14:16']);                                  
        VCSS_PostBack_DT      = reshape(VCSS_PostBack_DT, [npts 2]);
...
% Pre-allocate vectors for pertinent metrics
    % Date
    VCSS_PreB_Date                      = NaT(350,  npts);
    VCSS_ChamON_Date                    = NaT(350,  npts);
    VCSS_PostB_Date                     = NaT(350,  npts);
    % Seconds
    VCSS_PreB_sec                       = zeros(350, npts);
    VCSS_ChamON_sec                     = zeros(350, npts);
    VCSS_PostB_sec                      = zeros(350, npts);
    % HP_12CH4_dry
    VCSS_PreB_HP_12CH4                  = NaN(350,  npts);
    VCSS_ChamON_HP_12CH4                = NaN(350,  npts);
    VCSS_PostB_HP_12CH4                 = NaN(350,  npts);
    % HR_12CH4_dry
    VCSS_PreB_HR_12CH4                  = NaN(350, npts);
    VCSS_ChamON_HR_12CH4                = NaN(350, npts);
    VCSS_PostB_HR_12CH4                 = NaN(350, npts);
    % HP_Delta_iCH4_Raw
    VCSS_PreB_HP_Delta_iCH4             = NaN(350, npts);
    VCSS_ChamON_HP_Delta_iCH4           = NaN(350, npts);
    VCSS_PostB_HP_Delta_iCH4            = NaN(350, npts);
    % HR_Delta_iCH4_Raw
    VCSS_PreB_HR_Delta_iCH4             = NaN(350, npts);
    VCSS_ChamON_HR_Delta_iCH4           = NaN(350, npts);
    VCSS_PostB_HR_Delta_iCH4            = NaN(350, npts);
    % CO2_dry
    VCSS_PreB_CO2                       = NaN(350, npts);
    VCSS_ChamON_CO2                     = NaN(350, npts);
    VCSS_PostB_CO2                      = NaN(350, npts);
    % Delta_Raw_iCO2_Raw
    VCSS_PreB_Delta_iCO2                = NaN(350, npts);
    VCSS_ChamON_Delta_iCO2              = NaN(350, npts);
    VCSS_PostB_Delta_iCO2               = NaN(350, npts);
...
for idx = 1:npts
    subplot(3, 2, idx)
        scatter(VCSS_ISM_DATA(:,1,idx),                                 ...
                VCSS_ISM_DATA(:,2,idx))
        title_string = sprintf('[CH_4] vs. Time | Trans 01 | Pt. %d', idx);
        title(title_string)
        ylabel('[CH_4] (ppm)')
        xlabel('Time (hr)')
end
...
for idx = 1:npts
    subplot(3, 2, idx)
        scatter(VCSS_ISM_DATA(:,1,idx),                                 ...
                VCSS_ISM_DATA(:,6,idx), 'mo')
        title_string = sprintf('[CO_2] vs. Time | Trans 01 | Pt. %d', idx);
        title(title_string)
        ylabel('[CO_2] (ppm)')
        xlabel('Time (hr)')
end
...
%----||Linear Model Analysis||----%
    ...
    CH4_lin_mdl     = cell ([1 npts]); 
    CH4_lin_slope   = zeros([2 4 npts]);
    CH4_lin_flux    = zeros([1 npts]);
    ...
    for i = 1:npts
       ...
       CH4_lin_flux(i)          = CH4_lin_slope(2,1,i) .* H;
        ...
        %----Carbon Dioxide----%
    % Pre-allocate vector for fluxes (per measurement cycle)
    CO2_lin_mdl     = cell ([1 npts]); 
    CO2_lin_slope   = zeros([2 4 npts]);
    CO2_lin_flux    = zeros([1 npts]);
    % Use for-loop to run a linear regression for each of the chamber
    % measuerments
    for i = 1:npts
...
%----||Exponential Model Analysis||----%
    ...
    CH4_exp_slope   = zeros([1 3 npts]);
    CH4_exp_flux    = zeros([1 npts]);

    x_expData = cell([1 npts]);
    y_expData = cell([1 npts]);
...
clearvars x_expData y_expData x_exp_prepData y_exp_prepData
    ...
    %c_0         = 1.3575; % mg / m^3 
        % (This is equivalent to 1.9 ppm CH4 and could be adjusted to
        % indicate the concentration at time = 0)
    CO2_exp_slope   = zeros([1 3 npts]);
    CO2_exp_flux    = zeros([1 npts]);
...
CH4_KC_rslts    = cell([1 npts]);
CH4_KC_coeffs   = zeros([1 2 npts]);
CH4_KC_coeffs_ci= zeros([2 2 npts]);
...
%----Carbon Dioxide----%
CO2_KC_rslts    = cell([1 npts]);
CO2_KC_coeffs   = zeros([1 2 npts]);
CO2_KC_coeffs_ci= zeros([2 2 npts]);
...
for i = 1:npts
    title_string = sprintf('VCSS | CO_2:CH_4 Ratio vs. Time | Trans 021 | Pt %d', i);
    subplot(3,2,i)
...
Flx_Ratio = CH4_lin_flux ./ CO2_lin_flux;
    ...
    b1 = bar(Flx_Ratio, 'FaceColor', [0.3010 0.7450 0.9330],            ...
            'EdgeColor', 'k');
            title('CH_4-CO_2 Flux Ratio | VCSS | Trans 02')
            ylabel('F_{CH_4}:F_{CO_2}')
            xticklabels({'Point 01', 'Point 02', 'Point 03'});
    subplot(1,2,2)
    b2 = bar(log10(Flx_Ratio), 'FaceColor', [0.3010 0.7450 0.9330],            ...
            'EdgeColor', 'k');
            ylabel('log_{10}(F_{CH_4}:F_{CO_2}) (mg m^{-2} hr^{-1})')
            xticklabels({'Point 01', 'Point 02', 'Point 03'});
                     
                     
% Compare carbon isotopes for both gases
VCSS_isocomp    = zeros([1 npts]);
for i = 1:npts
...
if writeXL == 1
    for i = 1:npts
        export_chamON_data  = [VCSS_ISM_DATA(:,1,i), VCSS_ISM_DATA(:,2,i),   ...
                               ...
                               VCSS_ISM_DATA(:,9,i)];
        export_tabname          = sprintf('VCSS_Trans02_Pt%d', i);
        ...
        XL_filename             = 'C:/Users/moyoa/Google Drive/CompSci/PhD Dissertation/Data Analysis/Picarro/Valles/VCSS_Trans02_Jul2017.xlsx';
