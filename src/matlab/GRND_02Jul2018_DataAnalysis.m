...
% Metadata File: "...\Yellowstone\Jul2018\02Jul2018\GRND_02Jul2018\GRND_02Jul2018_Metadata"
...
% working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2018\02Jul2018\";
...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/02Jul2018/GRND_02Jul2018");
...
%% Gather the GRND_02Jul2018 Data from the Forerunner Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData    = addvars(TT_PicData,GRND_rel_sec,                    ...
                           'Before','HP_CH4_dry');
...
% the 0's will indicate rows that are to be removed.
    % For the GRND_02Jul2018, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), GRND_rel_sec                    ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_GRND*
    TT_GRND       = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),        ...
    ...
    smry_GRND_02Jul2018 = summary(TT_GRND);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_GRND.Time, TT_GRND.HP_CH4_dry, 'o')
        title('GRND 02Jul2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_GRND.Time, TT_GRND.CO2_dry, 'mo')
        title('GRND 02Jul2018 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_GRND.Time,TT_GRND.HP_CH4_dry, 'o');
                    title('GRND 02Jul2018 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_GRND.Time,TT_GRND.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
%% Extract the individual measurement
    ...
    GRND_PreBack    = NaN([115, width(TT_GRND)+1, nchams]); 
    GRND_ChamON     = NaN([400, width(TT_GRND)+1, nchams]);
    GRND_PostBack   = NaN([115, width(TT_GRND)+1, nchams]);
...
%% Extract  datetimes
    ...
    % PreBack
        % Pre-allocate
    GRND_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(GRND_PreBack(:,1,idx)));
    GRND_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(GRND_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    GRND_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(GRND_ChamON(:,1,idx)));
    GRND_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(GRND_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    GRND_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(GRND_PostBack(:,1,idx)));
    GRND_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(GRND_PostBack(1:min(f)-1,1,idx)));
end
...
[fig03, fig04, fig05] = basic_gas_viz(GRND_ChamON, nchams, site_tag,    ...
...
% c_CH4_ppm                       = GRND_ChamON(:,3,:);   
% c_CO2_ppm                       = GRND_ChamON(:,7,:);
% % Chameber Height (m)
%     % 5in = 12.7 cm = 0.127 m
% H                               = 0.127; 
...
%  for idx = 1:nchams
%         GRND_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
%         GRND_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
%  end
% 
% % Clear extraneous variables
...
GRND_ChamON = gas_unit_prep(GRND_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(GRND_ChamON(:,02,i),                  ...
                                  GRND_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ GRND_ChamON(:,3,idx)),           ...
                                         GRND_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ GRND_ChamON(:,7,idx)),         ...
                                         GRND_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(GRND_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(GRND_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = GRND_ChamON(:,5,:); iCO2 = GRND_ChamON(:,8,:);
...
GRND_ChamON_Ratio           = GRND_ChamON(:,7,:) ./ GRND_ChamON(:,3,:);
...
GRND_ChamON_Ratio_stats     = [mean(GRND_ChamON_Ratio);                 ...
                               median(GRND_ChamON_Ratio);               ...
                               range(GRND_ChamON_Ratio);                ...
                               std(GRND_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(GRND_ChamON(:,2,idx), GRND_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = GRND_ChamON(:,5,i); Y = GRND_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = GRND_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(GRND_ChamON_Ratio) max(GRND_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(GRND_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = GRND_ChamON(:,5,i);
        xyz(:,2,i) = GRND_ChamON(:,8,i);
        xyz(:,3,i) = GRND_ChamON_Ratio(:,1,i);
    end
...
GRND_PreBack(GRND_PreBack == 0)     = NaN;
GRND_PostBack(GRND_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(GRND_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(GRND_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(GRND_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(GRND_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(GRND_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(GRND_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(GRND_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(GRND_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GRND_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GRND_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GRND_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GRND_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GRND_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GRND_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GRND_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GRND_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    ...
    for i = 1:nchams
        ...
        export_ChamON_times     = string(GRND_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = GRND_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(GRND_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(GRND_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = GRND_PreBack (:,3,i);
        export_PreB_CO2         = GRND_PreBack (:,7,i);
        export_PreB_iCH4        = GRND_PreBack (:,5,i);
        export_PreB_iCO2        = GRND_PreBack (:,8,i);
        export_PostB_CH4        = GRND_PostBack(:,3,i);
        export_PostB_CO2        = GRND_PostBack(:,7,i);
        export_PostB_iCH4       = GRND_PostBack(:,5,i);
        export_PostB_iCO2       = GRND_PostBack(:,8,i);
        ...
        msg_str = sprintf('Tab ''GRND_%d'' has been created', i);
        disp(msg_str)
    end
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            'GRND_PreBack_DateTimes' , 'GRND_PreBack' , ...
                            'GRND_ChamON_DateTimes'  , 'GRND_ChamON'  , ...
                            'GRND_PostBack_DateTimes', 'GRND_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
