...
% working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2018\12Jun2019\";
...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/12Jun2019");
...
%% Gather the NGBN_STMB_12Jun2019 Data from the Forerunner Matrix 'PD_mtrx'
...
% the 0's will indicate rows that are to be removed.
    % For the NGBN_STMB_12Jun2019, the data presented groups of four thus, only
...
% Reassign the variables so that they represent the filtered information
    ...
    smry_NGBN_STMB_12Jun2019 = summary(TT_NGBN_STMB);
    % Clear extraneous variables
...
subplot(1,2,1)
    plot(TT_NGBN_STMB.Time, TT_NGBN_STMB.HP_CH4_dry, 'o')
        title('NGBN_STMB 12Jun2019 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_NGBN_STMB.Time, TT_NGBN_STMB.CO2_dry, 'mo')
        title('NGBN_STMB 12Jun2019 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
% --- CH4 --- %
   h1 = plot(TT_NGBN_STMB.Time,TT_NGBN_STMB.HP_CH4_dry, 'o');
                    title('NGBN_STMB 12Jun2019 [CH_4] and [CO_2] vs. Time')
                    grid on
...
for i = 1:nchams
   ...
   [m, n]       = size(A);
    ...
    NGBN_STMB_PostBack(1:m,1:n,i) = A;
...
for idx = 1:nchams
    ...
    % ChamON
        % Pre-allocate
    NGBN_STMB_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
    ...
    % PostBack
        % Pre-allocate
    NGBN_STMB_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(NGBN_STMB_PostBack(:,1,idx)));
    NGBN_STMB_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(NGBN_STMB_PostBack(1:min(f)-1,1,idx)));
end
...
NGBN_STMB_ChamON = gas_unit_prep(NGBN_STMB_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
NGBN_STMB_PostBack(NGBN_STMB_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(NGBN_STMB_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(NGBN_STMB_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(NGBN_STMB_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(NGBN_STMB_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(NGBN_STMB_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(NGBN_STMB_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(NGBN_STMB_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(NGBN_STMB_PostBack(:,8,i));   
    ...
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(NGBN_STMB_PostBack(:,7,i));
    ...
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(NGBN_STMB_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(NGBN_STMB_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            ...
                            'NGBN_STMB_PostBack_DateTimes', 'NGBN_STMB_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
