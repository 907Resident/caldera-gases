...
% Metadata File: "...\Yellowstone\Jul2018\04Jul2018\YSLK_04Jul2018_Metadata"
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jul2018\04Jul2018\";

% Date of measurements
ddmmmyyyy = "04Jul2018";

% Data should be imported via "Merge_PicData_Fxn"
 directory = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jul2018\04Jul2018\*.dat";
[TT_PicData,PD_mtrx_nm,PD_mtrx_dt, dte] =                               ...
 Merge_PicData_Fxn(directory, 'America/Denver');  

%% Gather the YSLK_04Jul2018 Data from the Eosense Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData          = addvars(TT_PicData,YSLK_rel_sec,              ...
                         'Before','HP_CH4_dry');
...
% the 0's will indicate rows that are to be removed.
    % For the YSLK_04Jul2018, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), YSLK_rel_sec          ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_YSLK_04Jul2018*
    TT_YSLK = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),    ...
    ...
    smry_YSLK_04Jul2018 = summary(TT_YSLK);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_YSLK.Time, TT_YSLK.HP_CH4_dry, 'o')
        title('YSLK 04Jul2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
...
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_YSLK.Time, TT_YSLK.CO2_dry, 'mo')
        title('YSLK 04Jul2018 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_YSLK.Time,TT_YSLK.HP_CH4_dry, 'o');
                    title('YSLK 04Jul2018 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_YSLK.Time,TT_YSLK.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
['12:28:29'; '13:08:04'; '14:16:48'; '15:03:00'; '15:39:12';            ...
 '16:14:14'],                                                           ...
...
['12:33:25'; '13:13:04'; '14:21:21'; '15:08:51'; '15:44:11';            ...
 '16:19:22'],                                                           ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['12:33:29'; '13:13:08'; '14:21:25'; '15:08:55'; '15:44:15';            ...
 '16:19:16'],                                                           ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['12:53:10'; '13:23:29'; '14:41:22'; '15:18:49'; '15:59:33';            ...
 '16:26:13'],                                                           ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['12:53:52'; '13:24:15'; '14:42:12'; '15:19:10'; '16:05:02';            ...
 '16:32:08'],                                                           ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['12:57:02'; '13:28:22'; '14:47:11'; '15:24:12'; '16:10:10';            ...
 '16:35:04'],                                                           ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
%% Extract the Individual Measurement
    ...
    YSLK_PreBack    = NaN([100, width(TT_YSLK)+1, nchams]);
    YSLK_ChamON     = NaN([420, width(TT_YSLK)+1, nchams]);
    YSLK_PostBack   = NaN([100, width(TT_YSLK)+1, nchams]);
...
%% Extract Datetimes
    ...
    % PreBack
        % Pre-allocate
    YSLK_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(YSLK_PreBack(:,1,idx)));
    YSLK_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(YSLK_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    YSLK_ChamON_DateTimes           = NaT([350 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(YSLK_ChamON(:,1,idx)));
    YSLK_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(YSLK_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    YSLK_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(YSLK_PostBack(:,1,idx)));
    YSLK_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(YSLK_PostBack(1:min(f)-1,1,idx)));
end
...
[fig03, fig04, fig05] = basic_gas_viz(YSLK_ChamON, nchams, site_tag,    ...
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(YSLK_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        YSLK_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        YSLK_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(YSLK_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        YSLK_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
c_CH4_ppm                       = YSLK_ChamON(:,3,:);   
c_CO2_ppm                       = YSLK_ChamON(:,7,:);
% Chamber Height (m)
...
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        YSLK_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
        YSLK_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
 end
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(YSLK_ChamON(:,02,i),                  ...
                                  YSLK_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ YSLK_ChamON(:,3,idx)),         ...
                                         YSLK_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ YSLK_ChamON(:,7,idx)),         ...
                                         YSLK_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(YSLK_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2] = last10_iso(YSLK_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = YSLK_ChamON(:,5,:); iCO2 = YSLK_ChamON(:,8,:);
...
YSLK_ChamON_Ratio           = YSLK_ChamON(:,7,:) ./ YSLK_ChamON(:,3,:);
...
YSLK_ChamON_Ratio_stats     = [mean(YSLK_ChamON_Ratio);                 ...
                               median(YSLK_ChamON_Ratio);               ...
                               range(YSLK_ChamON_Ratio);                ...
                               std(YSLK_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(YSLK_ChamON(:,2,idx), YSLK_ChamON_Ratio(:,idx), ...
        ...
        % numerical portion of each title in the array of plots
            if idx     <= nchams
                ...
                % represented)
                title_str   = sprintf(                                  ...
                             'YSLK %d.%d %s [CO_2]/[CH_4] vs. Rel Time',  ...
                              trans,pnt,ddmmmyyyy);
...
for i = 1:nchams
     ...
     X = YSLK_ChamON(:,5,i); Y = YSLK_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = YSLK_ChamON_Ratio(:,i);
    ...
    '%s %d.%d %s d^{13}C-CO_2 vs. d^{13}C-CH_4',                        ...
     ...
     colormap cool
     % Apply colorbar and label to last chart
        c = colorbar;
     if i == nchams        
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(YSLK_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = YSLK_ChamON(:,5,i);
        xyz(:,2,i) = YSLK_ChamON(:,8,i);
        xyz(:,3,i) = YSLK_ChamON_Ratio(:,1,i);
    end
    ...
    nexttile
       ...
       % Apply gridding on both axis to see the data better
        ...
        % numerical portion of each title in the array of plots
                ...
                % Title string (to be interated so the proper graph is
                % represented)
                title_str   = sprintf('%s %d.%d %s Conc and Isotope Data',...
                                       site_tag,trans, pnt, ddmmmyyyy);
...
for i = 1:nchams
    ...
    % This if-elseif-else block is used to appropriately designate the
        % numerical portion of each title in the array of plots
                ...
                % represented)
                title_str   = sprintf(                                  ...
                             'YSLK %d.%d %s Residual of Surface Fit', ...
                              trans,pnt,ddmmmyyyy);
                title(title_str, 'FontSize', 7)
                % Add labels to axis
...
YSLK_PreBack(YSLK_PreBack == 0)     = NaN;
YSLK_PostBack(YSLK_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(YSLK_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(YSLK_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(YSLK_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(YSLK_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(YSLK_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(YSLK_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(YSLK_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(YSLK_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(YSLK_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(YSLK_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(YSLK_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(YSLK_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(YSLK_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(YSLK_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(YSLK_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(YSLK_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    for i = 1:nchams
        ...
        export_ChamON_times     = string(YSLK_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = YSLK_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(YSLK_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(YSLK_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = YSLK_PreBack (:,3,i);
        export_PreB_CO2         = YSLK_PreBack (:,7,i);
        export_PreB_iCH4        = YSLK_PreBack (:,5,i);
        export_PreB_iCO2        = YSLK_PreBack (:,8,i);
        export_PostB_CH4        = YSLK_PostBack(:,3,i);
        export_PostB_CO2        = YSLK_PostBack(:,7,i);
        export_PostB_iCH4       = YSLK_PostBack(:,5,i);
        export_PostB_iCO2       = YSLK_PostBack(:,8,i);
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            'YSLK_PreBack_DateTimes' , 'YSLK_PreBack' , ...
                            'YSLK_ChamON_DateTimes'  , 'YSLK_ChamON'  , ...
                            'YSLK_PostBack_DateTimes', 'YSLK_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
