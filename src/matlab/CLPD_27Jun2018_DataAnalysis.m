...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/27Jun2018");
...
%% Gather the CLPD_27Jun2018 Data from the Forerunner Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData    = addvars(TT_PicData,CLPD_rel_sec,                    ...
                           'Before','HP_CH4_dry');
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), CLPD_rel_sec                    ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_CLPD*
    TT_CLPD       = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),        ...
    ...
    smry_CLPD_27Jun2018 = summary(TT_CLPD);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_CLPD.Time, TT_CLPD.HP_CH4_dry, 'o')
        title('CLPD 27Jun2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_CLPD.Time, TT_CLPD.CO2_dry, 'mo')
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_CLPD.Time,TT_CLPD.HP_CH4_dry, 'o');
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_CLPD.Time,TT_CLPD.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
%% Extract the individual measurement
    ...
    CLPD_PreBack    = NaN([115, width(TT_CLPD)+1, nchams]); 
    CLPD_ChamON     = NaN([400, width(TT_CLPD)+1, nchams]);
    CLPD_PostBack   = NaN([115, width(TT_CLPD)+1, nchams]);
...
for idx = 1:nchams
    ...
    % ChamON
        % Pre-allocate
    CLPD_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
...
CLPD_ChamON = gas_unit_prep(CLPD_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
if writeXL == 1
    ...
    for i = 1:nchams
        ...
        msg_str = sprintf('Tab ''CLPD_%d'' has been created', i);
        disp(msg_str)
    end
