...
%% CLPD_27Jun2018_DataAnalysis

% CLPD_27Jun2018_DataAnalysis
% Geoscientists: Ajayi, M. & Mans, W.
% Location: Clepsydra Geyser (CLPD), Yellowstone National Park, WY 
% [44.5511, -110.8062]
% Data Type: Transect Measurements 
% Metadata File: "...\Yellowstone\June2018\27Jun2018\CLPD_27Jun2018_Metadata"
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2018\27Jun2018\";
...
%% Gather the CLPD_27Jun2018 Data from the Eosense Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    s                           = ms ./ 1E03;
    CLPD_27Jun2018_rel_sec      = s;
    % Add the relative seconds vector to the timetable
    TT_PicData          = addvars(TT_PicData,CLPD_27Jun2018_rel_sec,    ...
                        'Before','HP_CH4_dry');
...
% the 0's will indicate rows that are to be removed.
    % For the CLPD_27Jun2018, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), CLPD_27Jun2018_rel_sec          ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_CLPD_27Jun2018*
    TT_CLPD_27Jun2018 = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),    ...
    ...
    smry_CLPD_27Jun2018 = summary(TT_CLPD_27Jun2018);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_CLPD_27Jun2018.Time, TT_CLPD_27Jun2018.HP_CH4_dry, 'o')
        title('CLPD 27Jun2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
...
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_CLPD_27Jun2018.Time, TT_CLPD_27Jun2018.CO2_dry, 'mo')
        title('CLPD 27Jun2018 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_CLPD_27Jun2018.Time,TT_CLPD_27Jun2018.HP_CH4_dry, 'o');
                    title('CLPD 27Jun2018 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_CLPD_27Jun2018.Time,TT_CLPD_27Jun2018.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
['12:45:04'; '13:38:33'; '14:26:50'; '15:26:04'; '16:05:51';            ...
 '16:41:03'],                                                           ...
...
['12:50:41'; '13:43:23'; '14:31:48'; '15:31:19'; '16:10:34';            ...
 '16:44:59'],                                                           ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['12:50:44';  '13:43:26'; '14:31:52'; '15:31:23'; '16:10:37';           ...
 '16:45:02'],                                                           ...
 'InputFormat', 'HH:mm:ss');
...
['13:11:38';  '14:02:59'; '14:52:47'; '15:52:40'; '16:31:06';           ...
 '17:05:58'],                                                           ...
 'InputFormat', 'HH:mm:ss');
...
['13:12:27';  '14:03:45'; '14:53:12'; '15:53:15'; '16:31:49';           ...
 '17:06:58'],                                                           ...                                                           ...
 'InputFormat', 'HH:mm:ss');
...
['13:15:55';  '14:06:52'; '14:58:12'; '15:57:51'; '16:36:14';           ...
 '17:10:05'],                                                           ...
 'InputFormat', 'HH:mm:ss');
...
%% Extract the Individual Measurement
    ...
    CLPD_PreBack    = NaN([100, width(TT_CLPD_27Jun2018)+1, nchams]);
    CLPD_ChamON     = NaN([420, width(TT_CLPD_27Jun2018)+1, nchams]);
    CLPD_PostBack   = NaN([100, width(TT_CLPD_27Jun2018)+1, nchams]);
    
for i = 1:nchams
    % PreBack
    S_Pre           = timerange(PreBack_Times(i,1), PreBack_Times(i,2), ...
                               'closed');
    T               = TT_CLPD_27Jun2018(S_Pre,:);
...
%% Extract Datetimes
    ...
    % PreBack
        % Pre-allocate
    CLPD_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(CLPD_PreBack(:,1,idx)));
    CLPD_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(CLPD_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    CLPD_ChamON_DateTimes           = NaT([350 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(CLPD_ChamON(:,1,idx)));
    CLPD_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(CLPD_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    CLPD_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(CLPD_PostBack(:,1,idx)));
    CLPD_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(CLPD_PostBack(1:min(f)-1,1,idx)));
end
...
[fig03, fig04, fig05] = basic_gas_viz(CLPD_ChamON, nchams, site_tag,    ...
...
% Clear extraneous variables
clearvars CH4_pl CO2_pl title_str pnt tran
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(CLPD_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        CLPD_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        CLPD_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(CLPD_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        CLPD_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
c_CH4_ppm                       = CLPD_ChamON(:,3,:);   
c_CO2_ppm                       = CLPD_ChamON(:,7,:);
% Chamber Height (m)
...
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        CLPD_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
        CLPD_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
 end
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(CLPD_ChamON(:,02,i),                  ...
                                  CLPD_ChamON(:,10,i));   
...
%       legend({'Linear Model', 'Exponential Model'}, 'Location', 'SW')
        xticks(1:nchams)
        xticklabels({'1.1', '1.2', '1.3', '1.4.1', '1.4.2', '1.4.3'})
        hold off
        % Density Plot
        nexttile
...
[xData, yData]  = prepareCurveData((1 ./ CLPD_ChamON(:,3,idx)),         ...
                                         CLPD_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ CLPD_ChamON(:,7,idx)),         ...
                                         CLPD_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(CLPD_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(CLPD_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = CLPD_ChamON(:,5,:); iCO2 = CLPD_ChamON(:,8,:);
...
CLPD_ChamON_Ratio           = CLPD_ChamON(:,7,:) ./ CLPD_ChamON(:,3,:);
...
CLPD_ChamON_Ratio_stats     = [mean(CLPD_ChamON_Ratio);                 ...
                               median(CLPD_ChamON_Ratio);               ...
                               range(CLPD_ChamON_Ratio);                ...
                               std(CLPD_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(CLPD_ChamON(:,2,idx), CLPD_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = CLPD_ChamON(:,5,i); Y = CLPD_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = CLPD_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(CLPD_ChamON_Ratio) max(CLPD_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(CLPD_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = CLPD_ChamON(:,5,i);
        xyz(:,2,i) = CLPD_ChamON(:,8,i);
        xyz(:,3,i) = CLPD_ChamON_Ratio(:,1,i);
    end
...
CLPD_PreBack(CLPD_PreBack == 0)     = NaN;
CLPD_PostBack(CLPD_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(CLPD_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(CLPD_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(CLPD_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(CLPD_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(CLPD_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(CLPD_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(CLPD_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(CLPD_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(CLPD_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(CLPD_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(CLPD_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(CLPD_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(CLPD_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(CLPD_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(CLPD_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(CLPD_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
    ...
    for i = 1:nchams
        % Create a string to label the  tab of the excel sheet
            ...
            elseif i            > 3
             ...
             % Title string (to be interated so the location is by the name
             % of the tab)
            export_tabname      = sprintf('%s_%d.%d.%d',                ...
                                           site_tag,trans,pnt,rep);                
            else
                trans           = 99;
                pnt             = 99;
            end
        ...
        export_ChamON_times     = string(CLPD_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = CLPD_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(CLPD_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(CLPD_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = CLPD_PreBack (:,3,i);
        export_PreB_CO2         = CLPD_PreBack (:,7,i);
        export_PreB_iCH4        = CLPD_PreBack (:,5,i);
        export_PreB_iCO2        = CLPD_PreBack (:,8,i);
        export_PostB_CH4        = CLPD_PostBack(:,3,i);
        export_PostB_CO2        = CLPD_PostBack(:,7,i);
        export_PostB_iCH4       = CLPD_PostBack(:,5,i);
        export_PostB_iCO2       = CLPD_PostBack(:,8,i);
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            'CLPD_PreBack_DateTimes' , 'CLPD_PreBack' , ...
                            'CLPD_ChamON_DateTimes'  , 'CLPD_ChamON'  , ...
                            'CLPD_PostBack_DateTimes', 'CLPD_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
