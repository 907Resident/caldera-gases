...
% working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\valles\Jun2018\26May2019\";
...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/valles/VCHB/26May2019");
...
%% Gather the VCHB_26May2019 Data from the Forerunner Matrix 'PD_mtrx'
...
%% Extract the individual measurement
    ...
    VCHB_PreBack    = NaN([115, width(TT_VCHB)+1, nchams]); 
    VCHB_ChamON     = NaN([500, width(TT_VCHB)+1, nchams]);
    VCHB_PostBack   = NaN([115, width(TT_VCHB)+1, nchams]);
...
for idx = 1:nchams
    ...
    % ChamON
        % Pre-allocate
    VCHB_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
...
VCHB_ChamON = gas_unit_prep(VCHB_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
for i = 1:nchams 
    ...
    hold on
        % Post-Background
        if nnz(~isnan(VCHB_PostBack(:,3,i))) <= 3
            continue
        end
...
if writeXL == 1
    ...
    for i = 1:nchams
        ...
        msg_str = sprintf('Tab ''VCHB_%d'' has been created', i);
        disp(msg_str)
    end
