...
%%VCHB_26May2019_DataAnalysis

%VCHB_26May2019_DataAnalysis
% Geoscientists: Ajayi, M. & Rahilly, K.
% Location: Hummingbird Camp, Valles Caldera, NM
% [35.826108, -106.656054]
% Data Type: Transect Measurements 
% Metadata File: "...\Valles\May2019\26May2019\VCHB_26May2019\VCHB_26May2019_Metadata"
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Valles\VCHB\";
...
%% Gather the VCHB_26May2019 Data from the Eosense Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData          = addvars(TT_PicData,VCHB_rel_sec,              ...
                                 'Before','HP_CH4_dry');
...
%% Import Temperature Data
    ...
    % WBAN:94173 at Valles Lake [44.54444°, -110.42111°] was used for
    % this project. The data was reduced using a python script

weather_file = "E:\moyoa\Documents\OneDrive - Vanderbilt\Python\Valles-Python\VCHB_26May2019_Weather.csv";
...
% the 0's will indicate rows that are to be removed.
    % For the VCHB_26May2019, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), VCHB_rel_sec                    ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_VCHB*
    TT_VCHB = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),    ...
    ...
    smry_VCHB_26May2019 = summary(TT_VCHB);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_VCHB.Time, TT_VCHB.HP_CH4_dry, 'o')
        title('VCHB 26May2019 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_VCHB.Time, TT_VCHB.CO2_dry, 'mo')
        title('VCHB 26May2019 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_VCHB.Time,TT_VCHB.HP_CH4_dry, 'o');
                    title('VCHB 26May2019 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_VCHB.Time,TT_VCHB.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
['11:19:26'; '11:34:59'; '11:56:42'; '12:13:55'; '12:40:50'; '12:58:00';...
 '13:21:45'; '13:34:52'; '15:05:55'],                                   ...
...
['11:24:44'; '11:40:40'; '12:02:14'; '12:18:03'; '12:45:56'; '13:02:03';...
 '13:25:28'; '14:39:31'; '15:09:06'],                                   ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['11:24:09'; '11:39:34'; '12:01:21'; '12:18:06'; '12:46:00'; '13:00:33';...
 '13:24:57'; '14:42:01'; '15:08:24'],                                   ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['11:25:33'; '11:41:15'; '12:03:30'; '12:19:34'; '12:47:13'; '13:03:02';...
 '13:27:20'; '14:51:14'; '15:10:17'],                                   ...
 'InputFormat', 'HH:mm:ss');
...
['11:29:21'; '11:45:12'; '12:08:19'; '12:24:25'; '12:51:01'; '13:07:16';...
 '13:31:38'; '14:56:36'; '15:10:10'],                                   ...
 'InputFormat', 'HH:mm:ss');

PostBack_Times(:,1)             = DATE + timeofday(PostBack_Times(:,1));

PostBack_Times(:,2)             = datetime(                             ...
['11:34:03'; '11:49:05'; '12:13:41'; '12:29:42'; '12:56:57'; '13:11:10';...
 '13:34:39'; '15:00:58'; '15:10:17'],                                   ...
 'InputFormat', 'HH:mm:ss');

PostBack_Times(:,2)             = DATE + timeofday(PostBack_Times(:,2));

PostBack_Times(9,:) = NaT;
...
%% Extract the Individual Measurement
    ...
    VCHB_PreBack    = NaN([100, width(TT_VCHB)+1, nchams]);
    VCHB_ChamON     = NaN([420, width(TT_VCHB)+1, nchams]);
    VCHB_PostBack   = NaN([100, width(TT_VCHB)+1, nchams]);
...
%% Extract Datetimes
    ...
    % PreBack
        % Pre-allocate
    VCHB_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(VCHB_PreBack(:,1,idx)));
    VCHB_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(VCHB_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    VCHB_ChamON_DateTimes           = NaT([200 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(VCHB_ChamON(:,1,idx)));
    VCHB_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(VCHB_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    VCHB_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(VCHB_PostBack(:,1,idx)));
    VCHB_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(VCHB_PostBack(1:min(f)-1,1,idx)));
end
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(VCHB_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        VCHB_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        VCHB_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(VCHB_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        VCHB_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
[fig03, fig04, fig05] = basic_gas_viz(VCHB_ChamON, nchams, site_tag,    ...
...
% Converting constants
    ...
    % Conditional in case of instances where Temp or Press is NaN
    if isnan(nanmean(Weather_Data.AmbientTemp)) ~= 1 &&                 ...
       isnan(nanmean(Weather_Data.Pressure))    ~= 1
      [~,Vm]                    = MolVol(Temp,Press,'C','mbar');
    else
        % Standard Molar Volume (mol L^-1)
        Vm                      = 22.71108;
    end
    % g/mol
CH4_molec_wt                    = 16.043;
...
c_CH4_ppm                       = VCHB_ChamON(:,3,:);   
c_CO2_ppm                       = VCHB_ChamON(:,7,:);
% Chamber Height (m)
...
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        VCHB_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
        VCHB_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
 end
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(VCHB_ChamON(:,02,i),                  ...
                                  VCHB_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ VCHB_ChamON(:,3,idx)),         ...
                                         VCHB_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ VCHB_ChamON(:,7,idx)),         ...
                                         VCHB_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(VCHB_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(VCHB_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = VCHB_ChamON(:,5,:); iCO2 = VCHB_ChamON(:,8,:);
...
VCHB_ChamON_Ratio           = VCHB_ChamON(:,7,:) ./ VCHB_ChamON(:,3,:);
...
VCHB_ChamON_Ratio_stats     = [mean(VCHB_ChamON_Ratio);                 ...
                               median(VCHB_ChamON_Ratio);               ...
                               range(VCHB_ChamON_Ratio);                ...
                               std(VCHB_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(VCHB_ChamON(:,2,idx), VCHB_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = VCHB_ChamON(:,5,i); Y = VCHB_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = VCHB_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(VCHB_ChamON_Ratio) max(VCHB_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(VCHB_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = VCHB_ChamON(:,5,i);
        xyz(:,2,i) = VCHB_ChamON(:,8,i);
        xyz(:,3,i) = VCHB_ChamON_Ratio(:,1,i);
    end
...
VCHB_PreBack(VCHB_PreBack == 0)     = NaN;
VCHB_PostBack(VCHB_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(VCHB_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(VCHB_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(VCHB_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(VCHB_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(VCHB_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(VCHB_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(VCHB_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(VCHB_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCHB_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    if all(isnan(VCHB_PostBack(:,3,i))) ~= 1        
    [f, xi, bw_post] = ksdensity(VCHB_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCHB_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    if all(isnan(VCHB_PostBack(:,3,i))) ~= 1
        [f, xi, bw_post] = ksdensity(VCHB_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    if all(isnan(VCHB_PostBack(:,3,i))) ~= 1           
    [f, xi, bw_pre] = ksdensity(VCHB_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(VCHB_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCHB_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    if all(isnan(VCHB_PostBack(:,3,i))) ~= 1          
    [f, xi, bw_post] = ksdensity(VCHB_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    for i = 1:nchams
        ...
        export_ChamON_times     = string(VCHB_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = VCHB_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(VCHB_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(VCHB_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = VCHB_PreBack (:,3,i);
        export_PreB_CO2         = VCHB_PreBack (:,7,i);
        export_PreB_iCH4        = VCHB_PreBack (:,5,i);
        export_PreB_iCO2        = VCHB_PreBack (:,8,i);
        export_PostB_CH4        = VCHB_PostBack(:,3,i);
        export_PostB_CO2        = VCHB_PostBack(:,7,i);
        export_PostB_iCH4       = VCHB_PostBack(:,5,i);
        export_PostB_iCO2       = VCHB_PostBack(:,8,i);
...
% other measurements at VCHB
TT_VCHB_26May2019 = TT_VCHB;
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            'VCHB_PreBack_DateTimes' , 'VCHB_PreBack' , ...
                            'VCHB_ChamON_DateTimes'  , 'VCHB_ChamON'  , ...
                            'VCHB_PostBack_DateTimes', 'VCHB_PostBack', ...
                            'TT_VCHB_26May2019'      ,                  ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
