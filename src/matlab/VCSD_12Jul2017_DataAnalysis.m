...
% working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\valles\Jun2018\12Jul2017\";
...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/valles/VCSD/12Jul2017");
...
%% Gather the VCSD_12Jul2017 Data from the Forerunner Matrix 'PD_mtrx'
...
%% Extract the individual measurement
    ...
    VCSD_PreBack    = NaN([115, width(TT_VCSD)+1, nchams]); 
    VCSD_ChamON     = NaN([500, width(TT_VCSD)+1, nchams]);
    VCSD_PostBack   = NaN([115, width(TT_VCSD)+1, nchams]);
...
for idx = 1:nchams
    ...
    % ChamON
        % Pre-allocate
    VCSD_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
...
VCSD_ChamON = gas_unit_prep(VCSD_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
if writeXL == 1
    ...
    for i = 1:nchams
        ...
        msg_str = sprintf('Tab ''VCSD_%d'' has been created', i);
        disp(msg_str)
    end
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            ...
                            'VCSD_PostBack_DateTimes', 'VCSD_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
