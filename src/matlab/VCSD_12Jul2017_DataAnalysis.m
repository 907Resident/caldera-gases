...
%% VCSD_12Jul2017_Data Analysis

% VCSD_12Jul2017_Data Analysis
% Geoscientists: Ajayi, M. & Ayers, J.
% Location: Soda Dam, Valles Caldera, NM (Valles Caldera Soda Dam = VCSD)
% Data Type: Transect 01 | Pts. 01-03 (measured on 12 July 2017)
%            Transect 02 | Pts. 04-06 (measured on 12 July 2017)
% Metadata File: "...\Valles\12July2017\Picarro_Metadata_12Jul2017.txt"

%% Import Data Pre-Proccessed Data
    
% Create a directory to save files 
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Valles\VCSD\";
...
%% Gather the VCSD_12Jul2017 Data from the Eosense Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData          = addvars(TT_PicData,VCSD_rel_sec,              ...
                        'Before','HP_CH4_dry');
...
% the 0's will indicate rows that are to be removed.
    % For the VCSD_12Jul2017, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), VCSD_rel_sec          ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_VCSD_12Jul2017*
    TT_VCSD = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),    ...
    ...
    smry_VCSD_12Jul2017 = summary(TT_VCSD);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_VCSD.Time, TT_VCSD.HP_CH4_dry, 'o')
        title('VCSD 12Jul2017 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_VCSD.Time, TT_VCSD.CO2_dry, 'mo')
        title('VCSD 12Jul2017 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_VCSD.Time,TT_VCSD.HP_CH4_dry, 'o');
                    title('VCSD 12Jul2017 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_VCSD.Time,TT_VCSD.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
['10:40:05'; '11:41:04'; '12:24:05'; '13:04:19'; '15:04:21'; '15:39:20'],  ...
...
['10:45:06'; '11:46:57'; '12:29:11'; '13:09:48'; '15:09:50'; '15:44:49'],  ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['10:45:03'; '11:47:00'; '12:29:14'; '13:09:48'; '15:09:53'; '15:44:53'],  ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['11:05:04'; '11:59:52'; '12:35:49'; '13:18:30'; '15:21:55'; '15:59:58'],  ...
 'InputFormat', 'HH:mm:ss');
...
['11:05:04'; '12:00:52'; '12:36:00'; '13:22:28'; '15:21:55'; '16:01:09'],  ...
 'InputFormat', 'HH:mm:ss');

PostBack_Times(:,1)             = DATE + timeofday(PostBack_Times(:,1));

PostBack_Times(:,2)             = datetime(                             ...
['11:10:05'; '12:05:05'; '12:41:14'; '13:27:46'; '15:26:56'; '16:05:07'],  ...
 'InputFormat', 'HH:mm:ss');
...
%% Extract the Individual Measurement
    ...
    VCSD_PreBack    = NaN([100, width(TT_VCSD)+1, nchams]);
    VCSD_ChamON     = NaN([420, width(TT_VCSD)+1, nchams]);
    VCSD_PostBack   = NaN([100, width(TT_VCSD)+1, nchams]);
...
%% Extract Datetimes
    ...
    % PreBack
        % Pre-allocate
    VCSD_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(VCSD_PreBack(:,1,idx)));
    VCSD_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(VCSD_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    VCSD_ChamON_DateTimes           = NaT([350 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(VCSD_ChamON(:,1,idx)));
    VCSD_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(VCSD_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    VCSD_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(VCSD_PostBack(:,1,idx)));
    VCSD_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(VCSD_PostBack(1:min(f)-1,1,idx)));
end
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(VCSD_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        VCSD_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        VCSD_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(VCSD_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        VCSD_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
[fig03, fig04, fig05] = basic_gas_viz(VCSD_ChamON, nchams, site_tag,    ...
...
c_CH4_ppm                       = VCSD_ChamON(:,3,:);   
c_CO2_ppm                       = VCSD_ChamON(:,7,:);
% Chamber Height (m)
...
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        VCSD_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
        VCSD_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
 end
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(VCSD_ChamON(:,02,i),                  ...
                                  VCSD_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ VCSD_ChamON(:,3,idx)),         ...
                                         VCSD_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ VCSD_ChamON(:,7,idx)),         ...
                                         VCSD_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(VCSD_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(VCSD_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = VCSD_ChamON(:,5,:); iCO2 = VCSD_ChamON(:,8,:);
...
VCSD_ChamON_Ratio           = VCSD_ChamON(:,7,:) ./ VCSD_ChamON(:,3,:);
...
VCSD_ChamON_Ratio_stats     = [mean(VCSD_ChamON_Ratio);                 ...
                               median(VCSD_ChamON_Ratio);               ...
                               range(VCSD_ChamON_Ratio);                ...
                               std(VCSD_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(VCSD_ChamON(:,2,idx), VCSD_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = VCSD_ChamON(:,5,i); Y = VCSD_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = VCSD_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(VCSD_ChamON_Ratio) max(VCSD_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(VCSD_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = VCSD_ChamON(:,5,i);
        xyz(:,2,i) = VCSD_ChamON(:,8,i);
        xyz(:,3,i) = VCSD_ChamON_Ratio(:,1,i);
    end
...
VCSD_PreBack(VCSD_PreBack == 0)     = NaN;
VCSD_PostBack(VCSD_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(VCSD_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(VCSD_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(VCSD_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(VCSD_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(VCSD_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(VCSD_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(VCSD_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(VCSD_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCSD_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(VCSD_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCSD_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(VCSD_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCSD_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(VCSD_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCSD_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(VCSD_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    for i = 1:nchams
        ...
        export_ChamON_times     = string(VCSD_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = VCSD_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(VCSD_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(VCSD_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = VCSD_PreBack (:,3,i);
        export_PreB_CO2         = VCSD_PreBack (:,7,i);
        export_PreB_iCH4        = VCSD_PreBack (:,5,i);
        export_PreB_iCO2        = VCSD_PreBack (:,8,i);
        export_PostB_CH4        = VCSD_PostBack(:,3,i);
        export_PostB_CO2        = VCSD_PostBack(:,7,i);
        export_PostB_iCH4       = VCSD_PostBack(:,5,i);
        export_PostB_iCO2       = VCSD_PostBack(:,8,i);
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                  'VCSD_PreBack_DateTimes'      , 'VCSD_PreBack' ,      ...
                  'VCSD_ChamON_DateTimes'       , 'VCSD_ChamON'  ,      ...
                  'lin_flux'                    , 'lin_slope'    ,      ...
                  'lin_mdl'                     ,                       ...
                  'Rsquared_CH4'                , 'Rsquared_CO2' ,      ...
                  'last10_d13CH4'               ,                       ...
                  'last10_d13CO2'               , 'TT_VCSD'      ,      ...
                  '-v7.3')
