...
%% MMMTH_15Jun2019_DataAnalysis

% MMMTH_15Jun2019_DataAnalysis
% Geoscientists: Ajayi, M. & Ayers, J.
% Location: Mammoth Hot Springs, Yellowstone National Park, WY 
% [44.966959, -110.705998]
% Data Type: Transect Measurements 
% Metadata File: "...\Yellowstone\Jun2019\15Jun2019\MMTH_15Jun2019\MMTH_15Jun2019_Metadata"

%% Extra Functions Used in this Script
% The following functions were custome made for this analysis by M. Ajayi.
% These files can also be found within the MATLAB-functions folder within
% the repository
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2019\15Jun2019\";
...
%% Gather the MMTH_15Jun2019 Data from the Eosense Matrix 'PD_mtrx'
...
weather_file = "E:\moyoa\Documents\OneDrive - Vanderbilt\Python\Yellowstone-Python\MMTH\MMTH_15Jun2019_Weather.csv";
...
% the 0's will indicate rows that are to be removed.
    % For the MMTH_15Jun2019, the data presented groups of four thus, only
...
% Reassign the variables so that they represent the filtered information
    ...
    smry_MMTH_15Jun2019 = summary(TT_MMTH);
    % Clear extraneous variables
...
subplot(1,2,1)
    plot(TT_MMTH.Time, TT_MMTH.HP_CH4_dry, 'o')
        title('MMTH 15Jun2019 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
...
subplot(1,2,2)
    plot(TT_MMTH.Time, TT_MMTH.CO2_dry, 'mo')
        title('MMTH 15Jun2019 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
% --- CH4 --- %
   h1 = plot(TT_MMTH.Time,TT_MMTH.HP_CH4_dry, 'o');
                    title('MMTH 15Jun2019 [CH_4] and [CO_2] vs. Time')
                    grid on
...
%% Extract the Individual Measurement
    ...
    MMTH_PreBack    = NaN([100, width(TT_MMTH)+1, nchams]);
    MMTH_ChamON     = NaN([420, width(TT_MMTH)+1, nchams]);
    MMTH_PostBack   = NaN([100, width(TT_MMTH)+1, nchams]);
...
for idx = 1:nchams
    ...
    % ChamON
        % Pre-allocate
    MMTH_ChamON_DateTimes           = NaT([350 1 nchams]);
for idx = 1:nchams
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(MMTH_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        MMTH_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        MMTH_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(MMTH_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        MMTH_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
srt_ChamONs = MMTH_ChamON_DateTimes(1,1,:);
    % Reshape to a size(MMTH_ChamON_DateTimes,3) x 1 vector
    srt_ChamONs =  reshape(srt_ChamONs,                                 ...
                          [size(MMTH_ChamON_DateTimes,3), 1]);
...
for i = 1:size(MMTH_ChamON_DateTimes,3)
    ...
    for j = 2:height(Weather_Data)
        ...
        if isbetween(srt_ChamONs(i),                                    ...
           ...
           ro_dte = datenum_round_off(datenum(MMTH_ChamON_DateTimes(i)),...
                                     'hour');
...
Vm = repmat(Vm,[length(MMTH_ChamON), 1]);
...
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        MMTH_ChamON(:,10,idx)   = c_CH4_mg_m3(:,1,idx);
        MMTH_ChamON(:,11,idx)   = c_CO2_mg_m3(:,1,idx);
 end
