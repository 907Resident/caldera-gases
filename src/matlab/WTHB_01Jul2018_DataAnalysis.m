...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jul2018\01Jul2018\";
...
%% Gather the WTHB_WTHB Data from the Eosense Matrix 'PD_mtrx'
...
% the 0's will indicate rows that are to be removed.
    % For the WTHB_WTHB, the data presented groups of four thus, only
...
% Reassign the variables so that they represent the filtered information
    ...
    smry_WTHB_WTHB = summary(TT_WTHB);
    % Clear extraneous variables
...
subplot(1,2,1)
    plot(TT_WTHB.Time, TT_WTHB.HP_CH4_dry, 'o')
        title('WTHB WTHB [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
...
subplot(1,2,2)
    plot(TT_WTHB.Time, TT_WTHB.CO2_dry, 'mo')
        title('WTHB WTHB [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
% --- CH4 --- %
   h1 = plot(TT_WTHB.Time,TT_WTHB.HP_CH4_dry, 'o');
                    title('WTHB WTHB [CH_4] and [CO_2] vs. Time')
                    grid on
