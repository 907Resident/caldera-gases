...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/23Jun2018");
...
%% Extract the individual measurement
    ...
    GBSN_PreBack    = NaN([115, width(TT_GBSN)+1, nchams]); 
    GBSN_ChamON     = NaN([400, width(TT_GBSN)+1, nchams]);
    GBSN_PostBack   = NaN([115, width(TT_GBSN)+1, nchams]);
...
for idx = 1:nchams
    ...
    % ChamON
        % Pre-allocate
    GBSN_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
...
GBSN_ChamON = gas_unit_prep(GBSN_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = GBSN_ChamON(:,5,:); iCO2 = GBSN_ChamON(:,8,:);
...
if writeXL == 1
    ...
    for i = 1:nchams
        ...
        msg_str = sprintf('Tab ''GBSN_%d'' has been created', i);
        disp(msg_str)
    end
