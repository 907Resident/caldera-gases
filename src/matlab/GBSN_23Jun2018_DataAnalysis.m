...
%% GBSN_23Jun2018_Data Analysis

% GBSN_23Jun2018_DataAnalysis
% Geoscientists: Ajayi, M. & Mans, W.
% Location: Terrace Springs (GBSN), Yellowstone National Park, WY
% Data Type: Transect Measurements 
% Metadata File: "...\Yellowstone\June2018\23June2018\GBSN_23Jun2018_Metadata"
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2018\23Jun2018\";
...
%% Gather the GBSN_23Jun2018 Data from the Forerunner Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData = addvars(TT_PicData,GBSN_rel_sec,                       ...
                        'Before','HP_CH4_dry');
...
% the 0's will indicate rows that are to be removed.
    % For the GBSN_23Jun2018, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), GBSN_rel_sec                    ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_GBSN_23Jun2018*
    TT_GBSN = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),              ...
    ...
    smry_GBSN_23Jun2018 = summary(TT_GBSN);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_GBSN.Time, TT_GBSN.HP_CH4_dry, 'o')
        title('GBSN 23Jun2018 | [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on

%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_GBSN.Time, TT_GBSN.CO2_dry, 'mo')
        title('GBSN 23Jun2018 | [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_GBSN.Time,TT_GBSN.HP_CH4_dry, 'o');
                    title('GBSN 23Jun2018 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_GBSN.Time,TT_GBSN.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
['12:35:12'; '13:16:02'; '13:52:01'; '14:36:01'; '15:28:54'; '16:05:38'],  ...
...
['12:42:59'; '13:23:15'; '13:59:07'; '14:46:37'; '15:34:41'; '16:11:26'],  ...
 'InputFormat', 'HH:mm:ss');

ChamON_Times(:,1)               = DATE + timeofday(ChamON_Times(:,1));
% Fill in with the "end" time of each chamber closure period (i.e. the time
% when the chamber opens
ChamON_Times(:,2)               = datetime(                             ...
['13:00:52'; '13:43:17'; '14:19:11'; '15:05:37'; '15:54:44'; '16:31:56'],  ...
 'InputFormat', 'HH:mm:ss');
...
['13:03:20'; '13:44:27'; '14:21:17'; '15:06:50'; '15:55:58'; '16:32:56'],  ...
 'InputFormat', 'HH:mm:ss');
...
%% Extract the individual measurement
    ...
    GBSN_PreBack    = NaN([100, width(TT_GBSN)+1, nchams]);
    GBSN_ChamON     = NaN([420, width(TT_GBSN)+1, nchams]);
    GBSN_PostBack   = NaN([100, width(TT_GBSN)+1, nchams]);
...
%% Extract datetimes
    ...
    % PreBack
        % Pre-allocate
    GBSN_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(GBSN_PreBack(:,1,idx)));
    GBSN_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(GBSN_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    GBSN_ChamON_DateTimes           = NaT([420 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(GBSN_ChamON(:,1,idx)));
    GBSN_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(GBSN_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    GBSN_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(GBSN_PostBack(:,1,idx)));
    GBSN_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(GBSN_PostBack(1:min(f)-1,1,idx)));
end
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(GBSN_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        GBSN_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        GBSN_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(GBSN_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        GBSN_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
[fig03, fig04, fig05] = basic_gas_viz(GBSN_ChamON, nchams, site_tag,    ...
...
c_CH4_ppm                       = GBSN_ChamON(:,3,:);   
c_CO2_ppm                       = GBSN_ChamON(:,7,:);
% Chameber Height (m)
    % 5in = 10.0 cm = 0.100 m
H                               = 0.100; 
...
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        GBSN_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
        GBSN_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
 end
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(GBSN_ChamON(:,02,i),                  ...
                                  GBSN_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ GBSN_ChamON(:,3,idx)),         ...
                                         GBSN_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ GBSN_ChamON(:,7,idx)),         ...
                                         GBSN_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(GBSN_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(GBSN_ChamON, "y");
...
GBSN_ChamON_Ratio           = GBSN_ChamON(:,7,:) ./ GBSN_ChamON(:,3,:);
...
GBSN_ChamON_Ratio_stats     = [mean(GBSN_ChamON_Ratio);                 ...
                               median(GBSN_ChamON_Ratio);               ...
                               range(GBSN_ChamON_Ratio);                ...
                               std(GBSN_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(GBSN_ChamON(:,2,idx), GBSN_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = GBSN_ChamON(:,5,i); Y = GBSN_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = GBSN_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(GBSN_ChamON_Ratio) max(GBSN_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(GBSN_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = GBSN_ChamON(:,5,i);
        xyz(:,2,i) = GBSN_ChamON(:,8,i);
        xyz(:,3,i) = GBSN_ChamON_Ratio(:,1,i);
    end
...
GBSN_PreBack(GBSN_PreBack == 0)     = NaN;
GBSN_PostBack(GBSN_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(GBSN_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(GBSN_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(GBSN_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(GBSN_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(GBSN_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(GBSN_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(GBSN_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(GBSN_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GBSN_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GBSN_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GBSN_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GBSN_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GBSN_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GBSN_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GBSN_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GBSN_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    for i = 1:nchams
        ...
        export_ChamON_times     = string(GBSN_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = GBSN_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(GBSN_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(GBSN_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = GBSN_PreBack (:,3,i);
        export_PreB_CO2         = GBSN_PreBack (:,7,i);
        export_PreB_iCH4        = GBSN_PreBack (:,5,i);
        export_PreB_iCO2        = GBSN_PreBack (:,8,i);
        export_PostB_CH4        = GBSN_PostBack(:,3,i);
        export_PostB_CO2        = GBSN_PostBack(:,7,i);
        export_PostB_iCH4       = GBSN_PostBack(:,5,i);
        export_PostB_iCO2       = GBSN_PostBack(:,8,i);
...
for i = 1:nchams
    ...
    filename = working_dir+"GBSN_23Jun2018_MATFILE.mat";
    save(filename,                                                      ...
                            'GBSN_PreBack_DateTimes' , 'GBSN_PreBack' , ...
                            'GBSN_ChamON_DateTimes'  , 'GBSN_ChamON'  , ...
                            'GBSN_PostBack_DateTimes', 'GBSN_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
