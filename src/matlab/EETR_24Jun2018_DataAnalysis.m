...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/24Jun2018");
...
%% Extract the individual measurement
    ...
    EETR_PreBack    = NaN([115, width(TT_EETR)+1, nchams]); 
    EETR_ChamON     = NaN([400, width(TT_EETR)+1, nchams]);
    EETR_PostBack   = NaN([115, width(TT_EETR)+1, nchams]);
...
for idx = 1:nchams
    ...
    % ChamON
        % Pre-allocate
    EETR_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
...
EETR_ChamON = gas_unit_prep(EETR_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
if writeXL == 1
    ...
    for i = 1:nchams
        ...
        msg_str = sprintf('Tab ''EETR_%d'' has been created', i);
        disp(msg_str)
    end
