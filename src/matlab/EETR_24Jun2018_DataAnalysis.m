...
%% EETR_24Jun2018_Data Analysis

% EETR_24Jun2018_DataAnalysis
% Geoscientists: Ajayi, M. & Mans, W.
% Location: East Entrance (EETR), Yellowstone National Park, WY
% Data Type: Transect Measurements 
% Metadata File: "...\Yellowstone\June2018\24Jun2018\EETR_24Jun2018_Metadata"
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2018\24Jun2018\";
...
%% Gather the EETR_24Jun2018 Data from the Forerunner Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData      = addvars(TT_PicData,EETR_rel_sec,                  ...
                        'Before','HP_CH4_dry');
    % Clear extraneous variables
    clearvars ms s

%% Filter out Erroneous Measurements
TT_PicData.HP_CH4_dry(TT_PicData.HP_CH4_dry >= 1E06)        = NaN;
TT_PicData.HR_CH4_dry(TT_PicData.HR_CH4_dry >= 1E06)        = NaN;
TT_PicData.HP_d13_CH4(TT_PicData.HP_d13_CH4 >= 1E06)        = NaN;
TT_PicData.CO2_dry(TT_PicData.CO2_dry       >= 1E06)        = NaN;
TT_PicData.d13_CO2(TT_PicData.d13_CO2       >= 1E06)        = NaN;
...
% the 0's will indicate rows that are to be removed.
    % For the EETR_24Jun2018, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), EETR_rel_sec          ...
...
% Reassign the variables so that they represent the filtered information
    ...
    VariableNames     = {'Rel_Sec','HP_CH4_dry','HR_CH4_dry',           ...
                         'HP_d13_CH4','HR_d13_CH4','CO2_dry','d13_CO2', ...
                         'Alarm_Status'};
    % Recreate Timetable with pared down data, now called
    % *TT_EETR_24Jun2018*
    TT_EETR = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),    ...
    ...
    smry_EETR_24Jun2018 = summary(TT_EETR);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_EETR.Time, TT_EETR.HP_CH4_dry, 'o')
        title('EETR 24Jun2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_EETR.Time, TT_EETR.CO2_dry, 'mo')
        title('EETR 24Jun2018 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_EETR.Time,TT_EETR.HP_CH4_dry, 'o');
                    title('EETR 24Jun2018 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_EETR.Time,TT_EETR.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
%% Organize Data into Measurement Categories
    % Seperate data into Pre-Background (PreBack), Chamber on (ChamON), and
    % Post-Background (PostBack)
...
['12:50:28'; '13:06:59'; '13:32:29'; '13:54:56'; '14:22:46'; '14:43:56';...
 '15:19:03'; '15:40:55'; '16:11:56'],                                   ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
PreBack_Times(:,1)              = DATE + timeofday(PreBack_Times(:,1));
    % Add five minutes to the timestamp in the second comlumn to account
    % for the complete PreBack period
PreBack_Times(:,2)              = PreBack_Times(:,1) + (5*minutes);

%----ChamON----%
ChamON_Times                    = NaT([nchams, 2]);

% The addition of 15 seconds was chosen arbitrarily as a good example of
% seperation between chamber open and chamber closed
ChamON_Times(:,1)               = datetime(                             ...
['12:55:57'; '13:12:43'; '13:38:46'; '13:58:52'; '14:28:30'; '14:49:50';...
 '15:25:39'; '15:46:11'; '16:17:01'],                                   ...
 'InputFormat', 'HH:mm:ss');

ChamON_Times(:,1)               = DATE + timeofday(ChamON_Times(:,1));

ChamON_Times(:,2)               = datetime(                             ...
['12:58:45'; '13:15:14'; '13:48:35'; '14:09:36'; '14:39:40'; '15:03:58';...
 '15:36:45'; '15:58:57'; '16:37:35'],                                   ...
 'InputFormat', 'HH:mm:ss');
...
%----PostBack----%
    % Post-Background times are nont hard-coded here; instead, they are
    % estimated with hardcoded intervals.
PostBack_Times                  = NaT([nchams, 2]);

PostBack_Times(:,1)             = ChamON_Times(:,2)    + (75*seconds);
PostBack_Times(:,2)             = PostBack_Times(:,1)  + (5*minutes);

%----Alarm_Times----%
    % The first column represents datetimes for alarms for CH4 (> 10 ppm)
    % and datetimes for CO2 alarms (>2500 ppm) in the second column for the
    % nine measurements made at this site
% Alarm_Times                     = datetime(                             ...
% ['12:58:49', '12:54:35';'13:15:14', '13:13:53'; NaT, '13:13:53';        ...
%   NaT, '13:45:03'; NaT, '14:06:01'; NaT, '14:35:38'; NaT, '15:00:10';   ...
%   NaT, '15:33:00'; NaT, '15:55:22'; NaT, '16:33:07'],                   ...
%  'InputFormat', 'HH:mm:ss');    
...
%% Extract the individual measurement
    ...
    EETR_PreBack    = NaN([100, width(TT_EETR)+1, nchams]);
    EETR_ChamON     = NaN([420, width(TT_EETR)+1, nchams]);
    EETR_PostBack   = NaN([100, width(TT_EETR)+1, nchams]);
...
%% Extract datetimes
    ...
    % PreBack
        % Pre-allocate
    EETR_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(EETR_PreBack(:,1,idx)));
    EETR_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(EETR_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    EETR_ChamON_DateTimes           = NaT([420 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(EETR_ChamON(:,1,idx)));
    EETR_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(EETR_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    EETR_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(EETR_PostBack(:,1,idx)));
    EETR_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(EETR_PostBack(1:min(f)-1,1,idx)));
end
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(EETR_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        EETR_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        EETR_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(EETR_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        EETR_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
[fig03, fig04, fig05] = basic_gas_viz(EETR_ChamON, nchams, site_tag,    ...
...
c_CH4_ppm                       = EETR_ChamON(:,3,:);   
c_CO2_ppm                       = EETR_ChamON(:,7,:);
% Chamber Height (m)
...
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        EETR_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
        EETR_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
 end
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(EETR_ChamON(:,02,i),                  ...
                                  EETR_ChamON(:,10,i));   
...
%         end
        ...
        xticklabels({'1.1.1','1.1.2','1.2.1','1.2.2','1.3.1','1.3.2',   ...
                    '1.4.1', '1.4.2', '1.5.1'})        
...
% Save figure as .fig to working directory
fi       = sprintf("MATLAB_figs\\%s_%s_CH4_and_CO2_fluxes_scatter_and_density.fig",     ...
                    site_tag, ddmmmyyyy);
fig_file = working_dir+fi;                
...
[xData, yData]  = prepareCurveData((1 ./ EETR_ChamON(:,3,idx)),         ...
                                         EETR_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ EETR_ChamON(:,7,idx)),         ...
                                         EETR_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(EETR_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(EETR_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = EETR_ChamON(:,5,:); iCO2 = EETR_ChamON(:,8,:);
...
EETR_ChamON_Ratio           = EETR_ChamON(:,7,:) ./ EETR_ChamON(:,3,:);
...
EETR_ChamON_Ratio_stats     = [mean(EETR_ChamON_Ratio);                 ...
                               median(EETR_ChamON_Ratio);               ...
                               range(EETR_ChamON_Ratio);                ...
                               std(EETR_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(EETR_ChamON(:,2,idx), EETR_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = EETR_ChamON(:,5,i); Y = EETR_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = EETR_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(EETR_ChamON_Ratio) max(EETR_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(EETR_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = EETR_ChamON(:,5,i);
        xyz(:,2,i) = EETR_ChamON(:,8,i);
        xyz(:,3,i) = EETR_ChamON_Ratio(:,1,i);
    end
...
EETR_PreBack(EETR_PreBack == 0)     = NaN;
EETR_PostBack(EETR_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(EETR_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(EETR_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(EETR_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(EETR_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(EETR_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(EETR_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(EETR_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(EETR_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(EETR_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(EETR_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(EETR_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(EETR_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(EETR_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(EETR_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(EETR_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(EETR_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    for i = 1:nchams
        ...
        export_PreB_Times       = string(EETR_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(EETR_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = EETR_PreBack (:,3,i);
        export_PreB_CO2         = EETR_PreBack (:,7,i);
        export_PreB_iCH4        = EETR_PreBack (:,5,i);
        export_PreB_iCO2        = EETR_PreBack (:,8,i);
        export_PostB_CH4        = EETR_PostBack(:,3,i);
        export_PostB_CO2        = EETR_PostBack(:,7,i);
        export_PostB_iCH4       = EETR_PostBack(:,5,i);
        export_PostB_iCO2       = EETR_PostBack(:,8,i);
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            'EETR_PreBack_DateTimes' , 'EETR_PreBack' , ...
                            'EETR_ChamON_DateTimes'  , 'EETR_ChamON'  , ...
                            'EETR_PostBack_DateTimes', 'EETR_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
