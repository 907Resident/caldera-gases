...
%% VCAC_27May2019_DataAnalysis

% VCAC_27May2019_DataAnalysis
% Geoscientists: Ajayi, M. & Rahily, K.
% Location: Alamo Canyon, Valles Caldera National Preserve, NM
% [35.918079, -106.600358]
% Data Type: Transect Measurements 
% Metadata File: "...\Valles\VCAC\11July2017\Picarro_Metadata_27May2019.txt"
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Valles\VCAC\27May2019\";
...
%% Gather the VCAC_27May2019 Data from the Eosense Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData          = addvars(TT_PicData,VCAC_rel_sec,              ...
                        'Before','HP_CH4_dry');
...
% the 0's will indicate rows that are to be removed.
    % For the VCAC_27May2019, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), VCAC_rel_sec                    ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_VCAC_27May2019*
    TT_VCAC = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),    ...
    ...
    smry_VCAC_27May2019 = summary(TT_VCAC);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_VCAC.Time, TT_VCAC.HP_CH4_dry, 'o')
        title('VCAC 27May2019 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_VCAC.Time, TT_VCAC.CO2_dry, 'mo')
        title('VCAC 27May2019 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_VCAC.Time,TT_VCAC.HP_CH4_dry, 'o');
                    title('VCAC 27May2019 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_VCAC.Time,TT_VCAC.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
['12:12:24'; '14:02:16'; '14:32:56'; '16:06:36'; '16:30:26'],           ...
...
['12:16:58'; '14:07:57'; '14:37:45'; '16:11:42'; '16:35:59'],           ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['12:17:01'; '14:08:00'; '14:37:48'; '16:11:45'; '16:36:03'],           ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['12:32:14'; '14:23:11'; '14:53:52'; '16:17:09'; '16:51:01'],           ...
 'InputFormat', 'HH:mm:ss');
...
['12:33:10'; '14:23:53'; '14:53:56'; '16:23:46'; '16:51:46'],           ...
 'InputFormat', 'HH:mm:ss');

PostBack_Times(:,1)             = DATE + timeofday(PostBack_Times(:,1));

PostBack_Times(:,2)             = datetime(                             ...
['12:37:38'; '14:28:28'; '14:54:51'; '16:28:10'; '16:54:26'],           ...
 'InputFormat', 'HH:mm:ss');
...
%% Extract the Individual Measurement
    ...
    VCAC_PreBack    = NaN([100, width(TT_VCAC)+1, nchams]);
    VCAC_ChamON     = NaN([420, width(TT_VCAC)+1, nchams]);
    VCAC_PostBack   = NaN([100, width(TT_VCAC)+1, nchams]);
...
%% Extract Datetimes
    ...
    % PreBack
        % Pre-allocate
    VCAC_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(VCAC_PreBack(:,1,idx)));
    VCAC_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(VCAC_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    VCAC_ChamON_DateTimes           = NaT([350 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(VCAC_ChamON(:,1,idx)));
    VCAC_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(VCAC_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    VCAC_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(VCAC_PostBack(:,1,idx)));
    VCAC_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(VCAC_PostBack(1:min(f)-1,1,idx)));
end
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(VCAC_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        VCAC_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        VCAC_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(VCAC_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        VCAC_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
[fig03, fig04, fig05] = basic_gas_viz(VCAC_ChamON, nchams, site_tag,    ...
...
c_CH4_ppm                       = VCAC_ChamON(:,3,:);   
c_CO2_ppm                       = VCAC_ChamON(:,7,:);
% Chamber Height (m)
...
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        VCAC_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
        VCAC_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
 end
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(VCAC_ChamON(:,02,i),                  ...
                                  VCAC_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ VCAC_ChamON(:,3,idx)),         ...
                                         VCAC_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ VCAC_ChamON(:,7,idx)),         ...
                                         VCAC_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(VCAC_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(VCAC_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = VCAC_ChamON(:,5,:); iCO2 = VCAC_ChamON(:,8,:);
...
VCAC_ChamON_Ratio           = VCAC_ChamON(:,7,:) ./ VCAC_ChamON(:,3,:);
...
VCAC_ChamON_Ratio_stats     = [mean(VCAC_ChamON_Ratio);                 ...
                               median(VCAC_ChamON_Ratio);               ...
                               range(VCAC_ChamON_Ratio);                ...
                               std(VCAC_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(VCAC_ChamON(:,2,idx), VCAC_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = VCAC_ChamON(:,5,i); Y = VCAC_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = VCAC_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(VCAC_ChamON_Ratio) max(VCAC_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(VCAC_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = VCAC_ChamON(:,5,i);
        xyz(:,2,i) = VCAC_ChamON(:,8,i);
        xyz(:,3,i) = VCAC_ChamON_Ratio(:,1,i);
    end
...
VCAC_PreBack(VCAC_PreBack == 0)     = NaN;
VCAC_PostBack(VCAC_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(VCAC_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(VCAC_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(VCAC_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(VCAC_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(VCAC_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(VCAC_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(VCAC_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(VCAC_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCAC_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(VCAC_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCAC_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(VCAC_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCAC_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(VCAC_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCAC_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(VCAC_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    for i = 1:nchams
        ...
        export_ChamON_times     = string(VCAC_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = VCAC_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(VCAC_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(VCAC_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = VCAC_PreBack (:,3,i);
        export_PreB_CO2         = VCAC_PreBack (:,7,i);
        export_PreB_iCH4        = VCAC_PreBack (:,5,i);
        export_PreB_iCO2        = VCAC_PreBack (:,8,i);
        export_PostB_CH4        = VCAC_PostBack(:,3,i);
        export_PostB_CO2        = VCAC_PostBack(:,7,i);
        export_PostB_iCH4       = VCAC_PostBack(:,5,i);
        export_PostB_iCO2       = VCAC_PostBack(:,8,i);
...
% other measurements at VCAC
TT_VCAC_27May2019 = TT_VCAC;
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            'VCAC_PreBack_DateTimes' , 'VCAC_PreBack' , ...
                            'VCAC_ChamON_DateTimes'  , 'VCAC_ChamON'  , ...
                            'VCAC_PostBack_DateTimes', 'VCAC_PostBack', ...
                            'TT_VCAC_27May2019'      ,                  ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
