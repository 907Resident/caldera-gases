...
% working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\valles\Jun2018\27May2019\";
...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/valles/VCAC/27May2019");
...
%% Gather the VCAC_27May2019 Data from the Forerunner Matrix 'PD_mtrx'
...
%% Extract the individual measurement
    ...
    VCAC_PreBack    = NaN([115, width(TT_VCAC)+1, nchams]); 
    VCAC_ChamON     = NaN([500, width(TT_VCAC)+1, nchams]);
    VCAC_PostBack   = NaN([115, width(TT_VCAC)+1, nchams]);
...
for idx = 1:nchams
    ...
    % ChamON
        % Pre-allocate
    VCAC_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
...
VCAC_ChamON = gas_unit_prep(VCAC_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
if writeXL == 1
    ...
    for i = 1:nchams
        ...
        msg_str = sprintf('Tab ''VCAC_%d'' has been created', i);
        disp(msg_str)
    end
