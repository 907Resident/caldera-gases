...
%% MMTH_03Jul2018_DataAnalysis

% MMTH_03Jul2018_DataAnalysis
% Geoscientists: Ajayi, M. & Mans, W.
% Location: Mammoth Hot Springs (Lower Terrace), Yellowstone National Park, WY 
% [44.9727, -110.7054]
% Data Type: Transect Measurements 
% Metadata File: "...\Yellowstone\Jul2018\03Jul2018\MMTH_03Jul2018\MMTH_03Jul2018_Metadata"
...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/03Jul2018/MMTH_03Jul2018");
...
%% Gather the MMTH_03Jul2018 Data from the Forerunner Matrix 'PD_mtrx'
...
% the 0's will indicate rows that are to be removed.
    % For the MMTH_03Jul2018, the data presented groups of four thus, only
...
% Reassign the variables so that they represent the filtered information
    ...
    smry_MMTH_03Jul2018 = summary(TT_MMTH);
    % Clear extraneous variables
...
subplot(1,2,1)
    plot(TT_MMTH.Time, TT_MMTH.HP_CH4_dry, 'o')
        title('MMTH 03Jul2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_MMTH.Time, TT_MMTH.CO2_dry, 'mo')
        title('MMTH 03Jul2018 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
% --- CH4 --- %
   h1 = plot(TT_MMTH.Time,TT_MMTH.HP_CH4_dry, 'o');
                    title('MMTH 03Jul2018 [CH_4] and [CO_2] vs. Time')
                    grid on
