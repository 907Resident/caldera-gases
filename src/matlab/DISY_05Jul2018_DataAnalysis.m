...
% Metadata File: "...\Yellowstone\Jul2018\05Jul2018\DISY_05Jul2018_Metadata"
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jul2018\05Jul2018\";

% Date of measurements
ddmmmyyyy = "05Jul2018";

% Data should be imported via "Merge_PicData_Fxn"
 directory = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jul2018\05Jul2018\*.dat";
[TT_PicData,PD_mtrx_nm,PD_mtrx_dt, dte] =                               ...
 Merge_PicData_Fxn(directory, 'America/Denver'); 

%% Gather the DISY_05Jul2018 Data from the Eosense Matrix 'PD_mtrx'
...
% the 0's will indicate rows that are to be removed.
    % For the DISY_05Jul2018, the data presented groups of four thus, only
...
% Reassign the variables so that they represent the filtered information
    ...
    smry_DISY_05Jul2018 = summary(TT_DISY);
    % Clear extraneous variables
...
subplot(1,2,1)
    plot(TT_DISY.Time, TT_DISY.HP_CH4_dry, 'o')
        title('DISY 05Jul2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
...
subplot(1,2,2)
    plot(TT_DISY.Time, TT_DISY.CO2_dry, 'mo')
        title('DISY 05Jul2018 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
% --- CH4 --- %
   h1 = plot(TT_DISY.Time,TT_DISY.HP_CH4_dry, 'o');
                    title('DISY 05Jul2018 [CH_4] and [CO_2] vs. Time')
                    grid on
...
['12:10:33'; '12:40:22'; '13:39:00'; '14:07:35'; '14:33:43'],           ...
...
['12:15:17'; '12:45:23'; '13:44:00'; '14:12:20'; '14:38:43'],           ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['12:15:21';  '12:45:27'; '13:44:04'; '14:12:24'; '14:38:46'],          ...
 'InputFormat', 'HH:mm:ss');
...
['12:28:17';  '13:01:28'; '13:58:38'; '14:28:02'; '14:53:12'],          ...
 'InputFormat', 'HH:mm:ss');
...
['12:28:59';  '13:02:24'; '13:59:20'; '14:28:37'; '14:53:44'],          ...
 'InputFormat', 'HH:mm:ss');

PostBack_Times(:,1)             = DATE + timeofday(PostBack_Times(:,1));

PostBack_Times(:,2)             = datetime(                             ...
['12:33:59';  '13:07:24'; '14:03:50'; '14:33:47'; '14:56:57'],          ...
 'InputFormat', 'HH:mm:ss');
...
%% Extract the Individual Measurement
    ...
    DISY_ChamON     = NaN([420, width(TT_DISY)+1, nchams]);
...
for idx = 1:nchams
    ...
    % ChamON
        % Pre-allocate
    DISY_ChamON_DateTimes           = NaT([350 1 nchams]);
for idx = 1:nchams
...
[fig03, fig04] = basic_gas_viz(DISY_ChamON, nchams, site_tag,           ...
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(DISY_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        DISY_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        DISY_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(DISY_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        DISY_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
    ...
    nexttile
        ...
        % numerical portion of each title in the array of plots
            if idx     <= nchams
                ...
                % represented)
                title_str   = sprintf(                                  ...
                             'DISY %d.%d %s [CO_2]/[CH_4] vs. Rel Time',  ...
                              trans,pnt,ddmmmyyyy);
