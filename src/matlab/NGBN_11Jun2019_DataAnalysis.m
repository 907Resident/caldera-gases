...
% working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2018\11Jun2019\";
...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/11Jun2019");
...
%% Gather the NGBN_11Jun2019 Data from the Forerunner Matrix 'PD_mtrx'
...
%% Extract the individual measurement
    ...
    NGBN_PreBack    = NaN([115, width(TT_NGBN)+1, nchams]); 
    NGBN_ChamON     = NaN([400, width(TT_NGBN)+1, nchams]);
    NGBN_PostBack   = NaN([115, width(TT_NGBN)+1, nchams]);
...
for idx = 1:nchams
    ...
    % ChamON
        % Pre-allocate
    NGBN_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
...
%  for idx = 1:nchams
%         NGBN_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
%         NGBN_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
%  end
% 
% % Clear extraneous variables
...
NGBN_ChamON = gas_unit_prep(NGBN_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
if writeXL == 1
    ...
    for i = 1:nchams
        ...
        msg_str = sprintf('Tab ''NGBN_%d'' has been created', i);
        disp(msg_str)
    end
