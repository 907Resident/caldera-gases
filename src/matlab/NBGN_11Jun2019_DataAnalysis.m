...
%% NGBN_11Jun2019_DataAnalysis

% NGBN_11Jun2019_DataAnalysis
% Geoscientists: Ajayi, M. & Ayers, J.
% Location: Norris Geyser Basin, Yellowstone National Park, WY 
% [44.728624, -110.703262]
% Data Type: Transect Measurements 
% Metadata File: "...\Yellowstone\Jun2019\11Jun2019\NGBN_11Jun2019\NGBN_11Jun2019_Metadata"
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2019\11Jun2019\";
...
%% Gather the NGBN_11Jun2019 Data from the Eosense Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData          = addvars(TT_PicData,NGBN_rel_sec,    ...
                         'Before','HP_CH4_dry');
...
weather_file = "E:\moyoa\Documents\OneDrive - Vanderbilt\Python\Yellowstone-Python\NGBN\NGBN_11Jun2019_Weather.csv";
...
% the 0's will indicate rows that are to be removed.
    % For the NGBN_11Jun2019, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), NGBN_rel_sec          ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_NGBN*
    TT_NGBN = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),    ...
    ...
    smry_NGBN_11Jun2019 = summary(TT_NGBN);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_NGBN.Time, TT_NGBN.HP_CH4_dry, 'o')
        title('NGBN 11Jun2019 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
...
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_NGBN.Time, TT_NGBN.CO2_dry, 'mo')
        title('NGBN 11Jun2019 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_NGBN.Time,TT_NGBN.HP_CH4_dry, 'o');
                    title('NGBN 11Jun2019 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_NGBN.Time,TT_NGBN.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
['10:58:05'; '11:37:02'; '12:35:56'; '13:19:20'],                       ...
...
['11:03:15'; '11:41:44'; '12:40:01'; '13:23:22'],                       ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['11:03:18'; '11:41:48'; '12:40:04'; '13:23:22'],                       ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['11:18:12'; '11:56:40'; '12:55:23'; '13:38:38'],                       ...
 'InputFormat', 'HH:mm:ss');
...
%% Extract the Individual Measurement
    ...
    NGBN_PreBack    = NaN([100, width(TT_NGBN)+1, nchams]);
    NGBN_ChamON     = NaN([420, width(TT_NGBN)+1, nchams]);
    NGBN_PostBack   = NaN([100, width(TT_NGBN)+1, nchams]);
...
%% Extract Datetimes
    ...
    % PreBack
        % Pre-allocate
    NGBN_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(NGBN_PreBack(:,1,idx)));
    NGBN_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(NGBN_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    NGBN_ChamON_DateTimes           = NaT([350 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(NGBN_ChamON(:,1,idx)));
    NGBN_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(NGBN_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    NGBN_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(NGBN_PostBack(:,1,idx)));
    NGBN_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(NGBN_PostBack(1:min(f)-1,1,idx)));
end
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(NGBN_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        NGBN_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        NGBN_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(NGBN_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        NGBN_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
[fig03, fig04, fig05] = basic_gas_viz(NGBN_ChamON, nchams, site_tag,    ...
...
srt_ChamONs = NGBN_ChamON_DateTimes(1,1,:);
    % Reshape to a size(NGBN_ChamON_DateTimes,3) x 1 vector
    srt_ChamONs =  reshape(srt_ChamONs,                                 ...
                          [size(NGBN_ChamON_DateTimes,3), 1]);
...
for i = 1:size(NGBN_ChamON_DateTimes,3)
    ...
    for j = 2:height(Weather_Data)
        ...
        if isbetween(srt_ChamONs(i),                                    ...
           ...
           ro_dte = datenum_round_off(datenum(NGBN_ChamON_DateTimes(i)),...
                                     'hour');
...
Vm = repmat(Vm,[length(NGBN_ChamON), 1]);
...
c_CH4_ppm                       = NGBN_ChamON(:,3,:);   
c_CO2_ppm                       = NGBN_ChamON(:,7,:);
% Chamber Height (m)
...
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        NGBN_ChamON(:,10,idx)   = c_CH4_mg_m3(:,1,idx);
        NGBN_ChamON(:,11,idx)   = c_CO2_mg_m3(:,1,idx);
 end
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(NGBN_ChamON(:,02,i),                  ...
                                  NGBN_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ NGBN_ChamON(:,3,idx)),         ...
                                         NGBN_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ NGBN_ChamON(:,7,idx)),         ...
                                         NGBN_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(NGBN_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(NGBN_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = NGBN_ChamON(:,5,:); iCO2 = NGBN_ChamON(:,8,:);
...
NGBN_ChamON_Ratio           = NGBN_ChamON(:,7,:) ./ NGBN_ChamON(:,3,:);
...
NGBN_ChamON_Ratio_stats     = [mean(NGBN_ChamON_Ratio);                 ...
                               median(NGBN_ChamON_Ratio);               ...
                               range(NGBN_ChamON_Ratio);                ...
                               std(NGBN_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(NGBN_ChamON(:,2,idx), NGBN_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = NGBN_ChamON(:,5,i); Y = NGBN_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = NGBN_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(NGBN_ChamON_Ratio) max(NGBN_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(NGBN_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = NGBN_ChamON(:,5,i);
        xyz(:,2,i) = NGBN_ChamON(:,8,i);
        xyz(:,3,i) = NGBN_ChamON_Ratio(:,1,i);
    end
...
NGBN_PreBack(NGBN_PreBack == 0)     = NaN;
NGBN_PostBack(NGBN_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(NGBN_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(NGBN_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(NGBN_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(NGBN_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(NGBN_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(NGBN_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(NGBN_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(NGBN_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(NGBN_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(NGBN_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(NGBN_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(NGBN_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(NGBN_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(NGBN_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(NGBN_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(NGBN_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    for i = 1:nchams
        ...
        export_ChamON_times     = string(NGBN_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = NGBN_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(NGBN_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(NGBN_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = NGBN_PreBack (:,3,i);
        export_PreB_CO2         = NGBN_PreBack (:,7,i);
        export_PreB_iCH4        = NGBN_PreBack (:,5,i);
        export_PreB_iCO2        = NGBN_PreBack (:,8,i);
        export_PostB_CH4        = NGBN_PostBack(:,3,i);
        export_PostB_CO2        = NGBN_PostBack(:,7,i);
        export_PostB_iCH4       = NGBN_PostBack(:,5,i);
        export_PostB_iCO2       = NGBN_PostBack(:,8,i);
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            'NGBN_PreBack_DateTimes' , 'NGBN_PreBack' , ...
                            'NGBN_ChamON_DateTimes'  , 'NGBN_ChamON'  , ...
                            'NGBN_PostBack_DateTimes', 'NGBN_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
