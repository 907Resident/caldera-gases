...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/22Jun2018");
...
%% Extract the individual measurement
    ...
    GVNT_PreBack    = NaN([115, width(TT_GVNT)+1, nchams]); 
    GVNT_ChamON     = NaN([400, width(TT_GVNT)+1, nchams]);
    GVNT_PostBack   = NaN([115, width(TT_GVNT)+1, nchams]);
...
for idx = 1:nchams
    ...
    % ChamON
        % Pre-allocate
    GVNT_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
...
GVNT_ChamON = gas_unit_prep(                                            ...
    GVNT_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
if writeXL == 1
    ...
    for i = 1:nchams
        ...
        msg_str = sprintf('Tab ''GVNT_%d'' has been created', i);
        disp(msg_str)
    end
