...
%% GVNT_22Jun2018_Data Analysis

% GVNT_22Jun2018_DataAnalysis
% Geoscientists: Ajayi, M. & Mans, W.
% Location: Gas Vents (GVNT), Yellowstone National Park, WY
% Data Type: Transect Measurements 
% Metadata File: To be created (09 Aug 2018)
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2018\22Jun2018\";
...
%% Gather the GVNT_22Jun2018 Data from the Forerunner Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData = addvars(TT_PicData,GVNT_rel_sec,                       ...
                        'Before','HP_CH4_dry');
...
% the 0's will indicate rows that are to be removed.
    % For the GVNT_22Jun2018, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), GVNT_rel_sec                    ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_GVNT*
    TT_GVNT = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),              ...
    ...
    smry_GVNT_22Jun2018 = summary(TT_GVNT);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_GVNT.Time, TT_GVNT.HP_CH4_dry, 'o')
        title('GVNT 22Jun2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_GVNT.Time, TT_GVNT.CO2_dry, 'mo')
        title('GVNT 22Jun2018 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_GVNT.Time,TT_GVNT.HP_CH4_dry, 'o');
                    title('GVNT 22Jun2018 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_GVNT.Time,TT_GVNT.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
['12:07:02'; '12:41:01'; '13:08:30'; '13:30:40'; '14:01:12'; '14:32:19';...
 '14:56:44'; '15:17:57'],                                               ...
...
['12:14:00'; '12:47:55'; '13:14:42'; '13:37:28'; '14:07:29'; '14:38:31';...
 '15:02:54'; '15:24:02'],                                               ...
 'InputFormat', 'HH:mm:ss');
...
['12:23:36'; '12:58:33'; '13:23:29'; '13:46:36'; '14:14:46'; '14:45:21';...
 '15:09:41'; '15:33:52'],                                               ...
 'InputFormat', 'HH:mm:ss');
...
%% Extract the individual measurement
    ...
    GVNT_PreBack    = NaN([100, width(TT_GVNT)+1, nchams]);
    GVNT_ChamON     = NaN([350, width(TT_GVNT)+1, nchams]);
    GVNT_PostBack   = NaN([100, width(TT_GVNT)+1, nchams]);
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(GVNT_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        GVNT_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        GVNT_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(GVNT_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        GVNT_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
%% Extract datetimes
    ...
    % PreBack
        % Pre-allocate
    GVNT_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(GVNT_PreBack(:,1,idx)));
    GVNT_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(GVNT_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    GVNT_ChamON_DateTimes           = NaT([350 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(GVNT_ChamON(:,1,idx)));
    GVNT_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(GVNT_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    GVNT_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(GVNT_PostBack(:,1,idx)));
    GVNT_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(GVNT_PostBack(1:min(f)-1,1,idx)));
end
...
[fig03, fig04, fig05] = basic_gas_viz(GVNT_ChamON, nchams, site_tag,    ...
...
c_CH4_ppm                       = GVNT_ChamON(:,3,:);   
c_CO2_ppm                       = GVNT_ChamON(:,7,:);
% Chameber Height (m)
    % 5in = 10.0 cm = 0.100 m
H                               = 0.100; 
...
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        GVNT_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
        GVNT_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
 end
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(GVNT_ChamON(:,02,i),                  ...
                                  GVNT_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ GVNT_ChamON(:,3,idx)),         ...
                                         GVNT_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ GVNT_ChamON(:,7,idx)),         ...
                                         GVNT_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(GVNT_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(GVNT_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = GVNT_ChamON(:,5,:); iCO2 = GVNT_ChamON(:,8,:);
...
GVNT_ChamON_Ratio           = GVNT_ChamON(:,7,:) ./ GVNT_ChamON(:,3,:);
...
GVNT_ChamON_Ratio_stats     = [mean(GVNT_ChamON_Ratio);                 ...
                               median(GVNT_ChamON_Ratio);               ...
                               range(GVNT_ChamON_Ratio);                ...
                               std(GVNT_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(GVNT_ChamON(:,2,idx), GVNT_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = GVNT_ChamON(:,5,i); Y = GVNT_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = GVNT_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(GVNT_ChamON_Ratio) max(GVNT_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(GVNT_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = GVNT_ChamON(:,5,i);
        xyz(:,2,i) = GVNT_ChamON(:,8,i);
        xyz(:,3,i) = GVNT_ChamON_Ratio(:,1,i);
    end
...
GVNT_PreBack(GVNT_PreBack == 0)     = NaN;
GVNT_PostBack(GVNT_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(GVNT_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(GVNT_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(GVNT_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(GVNT_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(GVNT_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(GVNT_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(GVNT_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(GVNT_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GVNT_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GVNT_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GVNT_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GVNT_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GVNT_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GVNT_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GVNT_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GVNT_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    for i = 1:nchams
        ...
        export_ChamON_times     = string(GVNT_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = GVNT_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(GVNT_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(GVNT_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = GVNT_PreBack (:,3,i);
        export_PreB_CO2         = GVNT_PreBack (:,7,i);
        export_PreB_iCH4        = GVNT_PreBack (:,5,i);
        export_PreB_iCO2        = GVNT_PreBack (:,8,i);
        export_PostB_CH4        = GVNT_PostBack(:,3,i);
        export_PostB_CO2        = GVNT_PostBack(:,7,i);
        export_PostB_iCH4       = GVNT_PostBack(:,5,i);
        export_PostB_iCO2       = GVNT_PostBack(:,8,i);
    ...
    % Export Data to Summary Tab
      ...
      export_SUMTAB_srt_enclosure_time =                                ...
                 reshape(string(GVNT_ChamON_DateTimes(1,1,:)), [nchams 1]);
         
      % Acquire end time index for each chamber enclosure
      j = 1;
      export_SUMTAB_end_enclosure_time = string();
      for idx = 1:nchams
          end_time_idx          = find(~isnan(GVNT_ChamON(:,1,idx)),1,  ...
                                        'last'); 
...
for i = 1:nchams
    ...
    filename = working_dir+"GVNT_22Jun2018_MATFILE.mat";
    save(filename,                                                      ...
                            'GVNT_PreBack_DateTimes' , 'GVNT_PreBack' , ...
                            'GVNT_ChamON_DateTimes'  , 'GVNT_ChamON'  , ...
                            'GVNT_PostBack_DateTimes', 'GVNT_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
