...
% Metadata File: "...\Valles\VCAC\11July2017\Picarro_Metadata_11Jul2017.txt"
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Valles\VCAC\11July2017\";
...
%% Gather the VCAC_11Jul2017 Data from the Eosense Matrix 'PD_mtrx'
...
% the 0's will indicate rows that are to be removed.
    % For the VCAC_11Jul2017, the data presented groups of four thus, only
...
% Reassign the variables so that they represent the filtered information
    ...
    smry_VCAC_11Jul2017 = summary(TT_VCAC);
    % Clear extraneous variables
...
subplot(1,2,1)
    plot(TT_VCAC.Time, TT_VCAC.HP_CH4_dry, 'o')
        title('VCAC 11Jul2017 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_VCAC.Time, TT_VCAC.CO2_dry, 'mo')
        title('VCAC 11Jul2017 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
% --- CH4 --- %
   h1 = plot(TT_VCAC.Time,TT_VCAC.HP_CH4_dry, 'o');
                    title('VCAC 11Jul2017 [CH_4] and [CO_2] vs. Time')
                    grid on
