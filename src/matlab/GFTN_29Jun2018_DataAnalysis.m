...
%% GFTN_29Jun2018_DataAnalysis

% GFTN_29Jun2018_DataAnalysis
% Geoscientists: Ajayi, M. & Mans, W.
% Location: Great Fountain Geyser, Yellowstone National Park, WY 
% [44.5452, -110.7864]
% Data Type: Transect Measurements 
% Metadata File: "...\Yellowstone\Jun2018\29Jun2018\GFTN_29Jun2018\GFTN_29Jun2018_Metadata"
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2018\29Jun2018\";
...
%% Gather the GFTN_29Jun2018 Data from the Eosense Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    s                           = ms ./ 1E03;
    GFTN_29Jun2018_rel_sec      = s;
    % Add the relative seconds vector to the timetable
    TT_PicData          = addvars(TT_PicData,GFTN_29Jun2018_rel_sec,    ...
                         'Before','HP_CH4_dry');
...
% the 0's will indicate rows that are to be removed.
    % For the GFTN_29Jun2018, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), GFTN_29Jun2018_rel_sec          ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_GFTN_29Jun2018*
    TT_GFTN_29Jun2018 = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),    ...
    ...
    smry_GFTN_29Jun2018 = summary(TT_GFTN_29Jun2018);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_GFTN_29Jun2018.Time, TT_GFTN_29Jun2018.HP_CH4_dry, 'o')
        title('GFTN 29Jun2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_GFTN_29Jun2018.Time, TT_GFTN_29Jun2018.CO2_dry, 'mo')
        title('GFTN 29Jun2018 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_GFTN_29Jun2018.Time,TT_GFTN_29Jun2018.HP_CH4_dry, 'o');
                    title('GFTN 29Jun2018 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_GFTN_29Jun2018.Time,TT_GFTN_29Jun2018.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
%% Extract the Individual Measurement
    ...
    GFTN_PreBack    = NaN([100, width(TT_GFTN_29Jun2018)+1, nchams]);
    GFTN_ChamON     = NaN([420, width(TT_GFTN_29Jun2018)+1, nchams]);
    GFTN_PostBack   = NaN([100, width(TT_GFTN_29Jun2018)+1, nchams]);
    
for i = 1:nchams
    % PreBack
    S_Pre           = timerange(PreBack_Times(i,1), PreBack_Times(i,2), ...
                               'closed');
    T               = TT_GFTN_29Jun2018(S_Pre,:);
...
%% Extract Datetimes
    ...
    % PreBack
        % Pre-allocate
    GFTN_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(GFTN_PreBack(:,1,idx)));
    GFTN_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(GFTN_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    GFTN_ChamON_DateTimes           = NaT([350 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(GFTN_ChamON(:,1,idx)));
    GFTN_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(GFTN_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    GFTN_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(GFTN_PostBack(:,1,idx)));
    GFTN_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(GFTN_PostBack(1:min(f)-1,1,idx)));
end
...
[fig03, fig04, fig05] = basic_gas_viz(GFTN_ChamON, nchams, site_tag,    ...
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(GFTN_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        GFTN_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        GFTN_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(GFTN_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        GFTN_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
c_CH4_ppm                       = GFTN_ChamON(:,3,:);   
c_CO2_ppm                       = GFTN_ChamON(:,7,:);
% Chamber Height (m)
...
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        GFTN_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
        GFTN_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
 end
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(GFTN_ChamON(:,02,i),                  ...
                                  GFTN_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ GFTN_ChamON(:,3,idx)),         ...
                                         GFTN_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ GFTN_ChamON(:,7,idx)),         ...
                                         GFTN_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(GFTN_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(GFTN_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = GFTN_ChamON(:,5,:); iCO2 = GFTN_ChamON(:,8,:);
...
GFTN_ChamON_Ratio           = GFTN_ChamON(:,7,:) ./ GFTN_ChamON(:,3,:);
...
GFTN_ChamON_Ratio_stats     = [mean(GFTN_ChamON_Ratio);                 ...
                               median(GFTN_ChamON_Ratio);               ...
                               range(GFTN_ChamON_Ratio);                ...
                               std(GFTN_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(GFTN_ChamON(:,2,idx), GFTN_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = GFTN_ChamON(:,5,i); Y = GFTN_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = GFTN_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(GFTN_ChamON_Ratio) max(GFTN_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(GFTN_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = GFTN_ChamON(:,5,i);
        xyz(:,2,i) = GFTN_ChamON(:,8,i);
        xyz(:,3,i) = GFTN_ChamON_Ratio(:,1,i);
    end
...
GFTN_PreBack(GFTN_PreBack == 0)     = NaN;
GFTN_PostBack(GFTN_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(GFTN_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(GFTN_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(GFTN_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(GFTN_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(GFTN_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(GFTN_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(GFTN_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(GFTN_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GFTN_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GFTN_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GFTN_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GFTN_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GFTN_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GFTN_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(GFTN_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(GFTN_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    for i = 1:nchams
        ...
        export_ChamON_times     = string(GFTN_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = GFTN_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(GFTN_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(GFTN_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = GFTN_PreBack (:,3,i);
        export_PreB_CO2         = GFTN_PreBack (:,7,i);
        export_PreB_iCH4        = GFTN_PreBack (:,5,i);
        export_PreB_iCO2        = GFTN_PreBack (:,8,i);
        export_PostB_CH4        = GFTN_PostBack(:,3,i);
        export_PostB_CO2        = GFTN_PostBack(:,7,i);
        export_PostB_iCH4       = GFTN_PostBack(:,5,i);
        export_PostB_iCO2       = GFTN_PostBack(:,8,i);
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            'GFTN_PreBack_DateTimes' , 'GFTN_PreBack' , ...
                            'GFTN_ChamON_DateTimes'  , 'GFTN_ChamON'  , ...
                            'GFTN_PostBack_DateTimes', 'GFTN_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
