...
%% VCRP_13Jul2017_Data Analysis

% VCRP_13Jul2017_Data Analysis Geoscientists: Ajayi, M. & Ayers, J.
% Location: Redondo Peak, Valles Caldera, NM (Valles Caldera Redondo Peak =
% VCRP)
% Data Type: Transect 01 | Pts. 01-04 (measured on 13 July 2017)
%            Transect 02 | Pts. 05-07 (measured on 13 July 2017)
% Metadata File: "...\Valles\13July2017\Picarro_Metadata_13Jul2017.txt"
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Valles\VCRP\";
...
%% Gather the VCRP_13Jul2018 Data from the Eosense Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData          = addvars(TT_PicData,VCRP_rel_sec,              ...
                        'Before','HP_CH4_dry');
...
TT_PicData.CO2_dry(TT_PicData.CO2_dry       >= 4.50E03)         = NaN;
...
% the 0's will indicate rows that are to be removed.
    % For the VCRP_13Jul2018, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), VCRP_rel_sec          ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_VCRP_13Jul2018*
    TT_VCRP = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),    ...
    ...
    smry_VCRP_13Jul2018 = summary(TT_VCRP);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_VCRP.Time, TT_VCRP.HP_CH4_dry, 'o')
        title('VCRP 13Jul2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_VCRP.Time, TT_VCRP.CO2_dry, 'mo')
        title('VCRP 13Jul2018 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_VCRP.Time,TT_VCRP.HP_CH4_dry, 'o');
                    title('VCRP 13Jul2018 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_VCRP.Time,TT_VCRP.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
['11:58:36'; '12:32:12'; '13:06:05'; '13:41:46'; '14:49:24'; '15:35:23';...
 '16:08:31'],                                                           ...
...
['12:03:40'; '12:37:13'; '13:13:25'; '13:46:50'; '14:53:27'; '15:40:22';...
 '16:13:28'],                                                           ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['12:03:44'; '12:38:52'; '13:12:57'; '13:46:18'; '14:55:23'; '15:40:08';...
 '16:15:05'],                                                           ...
 'InputFormat', 'HH:mm:ss');
ChamON_Times(:,1)               = DATE + timeofday(ChamON_Times(:,1));

ChamON_Times(:,2)               = datetime(                             ...
['12:06:57'; '12:39:45'; '13:15:10'; '13:48:35'; '15:14:54'; '15:48:37';...
 '16:32:09'],                                                           ...
 'InputFormat', 'HH:mm:ss');
...
['12:13:11'; '12:44:31'; '13:22:15'; '13:55:56'; '15:14:58'; '15:55:58';...
 '16:33:56'],                                                           ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['12:18:09'; '12:49:23'; '13:26:20'; '13:59:20'; '15:19:20'; '15:59:58';...
 '16:35:41'],                                                           ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
%% Extract the Individual Measurement
    ...
    VCRP_PreBack    = NaN([100, width(TT_VCRP)+1, nchams]);
    VCRP_ChamON     = NaN([420, width(TT_VCRP)+1, nchams]);
    VCRP_PostBack   = NaN([100, width(TT_VCRP)+1, nchams]);
...
%% Extract Datetimes
    ...
    % PreBack
        % Pre-allocate
    VCRP_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(VCRP_PreBack(:,1,idx)));
    VCRP_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(VCRP_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    VCRP_ChamON_DateTimes           = NaT([350 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(VCRP_ChamON(:,1,idx)));
    VCRP_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(VCRP_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    VCRP_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(VCRP_PostBack(:,1,idx)));
    VCRP_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(VCRP_PostBack(1:min(f)-1,1,idx)));
end
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(VCRP_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        VCRP_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        VCRP_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(VCRP_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        VCRP_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
[fig03, fig04, fig05] = basic_gas_viz(VCRP_ChamON, nchams, site_tag,    ...
...
c_CH4_ppm                       = VCRP_ChamON(:,3,:);   
c_CO2_ppm                       = VCRP_ChamON(:,7,:);
% Chamber Height (m)
...
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        VCRP_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
        VCRP_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
 end
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(VCRP_ChamON(:,02,i),                  ...
                                  VCRP_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ VCRP_ChamON(:,3,idx)),         ...
                                         VCRP_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ VCRP_ChamON(:,7,idx)),         ...
                                         VCRP_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(VCRP_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(VCRP_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = VCRP_ChamON(:,5,:); iCO2 = VCRP_ChamON(:,8,:);
...
VCRP_ChamON_Ratio           = VCRP_ChamON(:,7,:) ./ VCRP_ChamON(:,3,:);
...
VCRP_ChamON_Ratio_stats     = [mean(VCRP_ChamON_Ratio);                 ...
                               median(VCRP_ChamON_Ratio);               ...
                               range(VCRP_ChamON_Ratio);                ...
                               std(VCRP_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(VCRP_ChamON(:,2,idx), VCRP_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = VCRP_ChamON(:,5,i); Y = VCRP_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = VCRP_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(VCRP_ChamON_Ratio) max(VCRP_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(VCRP_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = VCRP_ChamON(:,5,i);
        xyz(:,2,i) = VCRP_ChamON(:,8,i);
        xyz(:,3,i) = VCRP_ChamON_Ratio(:,1,i);
    end
...
VCRP_PreBack(VCRP_PreBack == 0)     = NaN;
VCRP_PostBack(VCRP_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(VCRP_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(VCRP_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(VCRP_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(VCRP_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(VCRP_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(VCRP_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(VCRP_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(VCRP_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCRP_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(VCRP_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCRP_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(VCRP_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCRP_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(VCRP_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(VCRP_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(VCRP_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    for i = 1:nchams
        ...
        export_ChamON_times     = string(VCRP_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = VCRP_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(VCRP_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(VCRP_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = VCRP_PreBack (:,3,i);
        export_PreB_CO2         = VCRP_PreBack (:,7,i);
        export_PreB_iCH4        = VCRP_PreBack (:,5,i);
        export_PreB_iCO2        = VCRP_PreBack (:,8,i);
        export_PostB_CH4        = VCRP_PostBack(:,3,i);
        export_PostB_CO2        = VCRP_PostBack(:,7,i);
        export_PostB_iCH4       = VCRP_PostBack(:,5,i);
        export_PostB_iCO2       = VCRP_PostBack(:,8,i);
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                  'VCRP_PreBack_DateTimes'      , 'VCRP_PreBack' ,      ...
                  'VCRP_ChamON_DateTimes'       , 'VCRP_ChamON'  ,      ...
                  'lin_flux'                    , 'lin_slope'         , ...
                  'lin_mdl'                     ,                       ...
                  'Rsquared_CH4'                , 'Rsquared_CO2'      , ...
                  'last10_d13CH4'               ,                       ...
                  'last10_d13CO2'               , 'TT_VCRP'      , ...
                  '-v7.3')