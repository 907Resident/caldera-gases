...
%% WTHB_07Jun2019_DataAnalysis

% WTHB_07Jun2019_DataAnalysis
% Geoscientists: Ajayi, M. & Ayers, J.
% Location: West Thumb Geyser Basin, Yellowstone National Park, WY 
% [44.416583, -110.571653]
% Data Type: Transect Measurements 
% Metadata File: "...\Yellowstone\Jun2019\07Jun2019\WTHB_07Jun2019\WTHB_07Jun2019_Metadata"

%% Extra Functions Used in this Script
    % The following functions were custome made for this analysis by M.
    % Ajayi
...
working_dir = "E:\moyoa\Documents\OneDrive - Vanderbilt\PhD_Dissertation\Data_Analysis\Picarro\Yellowstone\Jun2019\07Jun2019\";
...
%% Gather the WTHB_07Jun2019 Data from the Eosense Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData          = addvars(TT_PicData,WTHB_rel_sec,              ...
                         'Before','HP_CH4_dry');
...
weather_file = "E:\moyoa\Documents\OneDrive - Vanderbilt\Python\Yellowstone-Python\WTHB\WTHB_07Jun2019_Weather.csv";
...
% Create Dummy Table to create inner join
% WTHB_dates              = table;
% WTHB_dates_max          = max(TT_PicData.Time);
% WTHB_dates.DATETIME     = reshape(WTHB_dates_max, [nchams, 1]);
% WTHB_dates.DATETIME     = dateshift(WTHB_dates, 'start', 'hour');
% % Inner Join to complete slice of data 
% Weather_Data            = innerjoin(WTHB_dates, Weather_Data);
% 
% % Clear extraneous variables
% clearvars WTHB_dates_max WTHB_dates
...
% the 0's will indicate rows that are to be removed.
    % For the WTHB_07Jun2019, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), WTHB_rel_sec                    ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_WTHB_07Jun2019*
    TT_WTHB = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),    ...
    ...
    smry_WTHB_07Jun2019 = summary(TT_WTHB);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_WTHB.Time, TT_WTHB.HP_CH4_dry, 'o')
        title('WTHB 07Jun2019 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_WTHB.Time, TT_WTHB.CO2_dry, 'mo')
        title('WTHB 07Jun2019 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_WTHB.Time,TT_WTHB.HP_CH4_dry, 'o');
                    title('WTHB 07Jun2019 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_WTHB.Time,TT_WTHB.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
%% Extract the Individual Measurement
    ...
    WTHB_PreBack    = NaN([100, width(TT_WTHB)+1, nchams]);
    WTHB_ChamON     = NaN([420, width(TT_WTHB)+1, nchams]);
    WTHB_PostBack   = NaN([100, width(TT_WTHB)+1, nchams]);
...
%% Extract Datetimes
    ...
    % PreBack
        % Pre-allocate
    WTHB_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(WTHB_PreBack(:,1,idx)));
    WTHB_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(WTHB_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    WTHB_ChamON_DateTimes           = NaT([350 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(WTHB_ChamON(:,1,idx)));
    WTHB_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(WTHB_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    WTHB_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(WTHB_PostBack(:,1,idx)));
    WTHB_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(WTHB_PostBack(1:min(f)-1,1,idx)));
end
...
% CH4 Spec Check
for sp_idx = 1:nchams
    CH4_Sp_Chk = find(WTHB_ChamON(:,3,sp_idx) > 10);
    if  any(CH4_Sp_Chk)
        WTHB_ChamON(CH4_Sp_Chk,3,sp_idx) = NaN;
        WTHB_ChamON(CH4_Sp_Chk,5,sp_idx) = NaN;
    else
...
% CO2 Spec Check
for sp_idx = 1:nchams
    CO2_Sp_Chk = find(WTHB_ChamON(:,7,sp_idx) > 2500);
    if  any(CO2_Sp_Chk)
        WTHB_ChamON(CO2_Sp_Chk,7:8,sp_idx) = NaN;
    else
...
[fig03, fig04, fig05] = basic_gas_viz(WTHB_ChamON, nchams, site_tag,    ...
...
srt_ChamONs = WTHB_ChamON_DateTimes(1,1,:);
    % Reshape to a size(WTHB_ChamON_DateTimes,3) x 1 vector
    srt_ChamONs =  reshape(srt_ChamONs,                                 ...
                          [size(WTHB_ChamON_DateTimes,3), 1]);
...
for i = 1:size(WTHB_ChamON_DateTimes,3)
    ...
    for j = 2:height(Weather_Data)
        ...
        if isbetween(srt_ChamONs(i),                                    ...
           ...
           ro_dte = datenum_round_off(datenum(WTHB_ChamON_DateTimes(i)),...
                                     'hour');
...
c_CH4_ppm                       = WTHB_ChamON(:,3,:);   
c_CO2_ppm                       = WTHB_ChamON(:,7,:);
% Chamber Height (m)
...
% Add the converted cgs concentrations to the main numeric array
 for idx = 1:nchams
        WTHB_ChamON(:,10,idx)    = c_CH4_cgs(:,1,idx);
        WTHB_ChamON(:,11,idx)    = c_CO2_cgs(:,1,idx);
 end
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(WTHB_ChamON(:,02,i),                  ...
                                  WTHB_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ WTHB_ChamON(:,3,idx)),         ...
                                         WTHB_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ WTHB_ChamON(:,7,idx)),         ...
                                         WTHB_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[fig09, fig10] = kp_viz(WTHB_ChamON, nchams,                            ...
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(WTHB_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = WTHB_ChamON(:,5,:); iCO2 = WTHB_ChamON(:,8,:);
...
WTHB_ChamON_Ratio           = WTHB_ChamON(:,7,:) ./ WTHB_ChamON(:,3,:);
...
WTHB_ChamON_Ratio_stats     = [mean(WTHB_ChamON_Ratio);                 ...
                               median(WTHB_ChamON_Ratio);               ...
                               range(WTHB_ChamON_Ratio);                ...
                               std(WTHB_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(WTHB_ChamON(:,2,idx), WTHB_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = WTHB_ChamON(:,5,i); Y = WTHB_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = WTHB_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(WTHB_ChamON_Ratio) max(WTHB_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(WTHB_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = WTHB_ChamON(:,5,i);
        xyz(:,2,i) = WTHB_ChamON(:,8,i);
        xyz(:,3,i) = WTHB_ChamON_Ratio(:,1,i);
    end
...
WTHB_PreBack(WTHB_PreBack == 0)     = NaN;
WTHB_PostBack(WTHB_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(WTHB_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(WTHB_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(WTHB_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(WTHB_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(WTHB_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(WTHB_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(WTHB_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(WTHB_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(WTHB_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(WTHB_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(WTHB_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(WTHB_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(WTHB_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(WTHB_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(WTHB_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(WTHB_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    for i = 1:nchams
        ...
        export_ChamON_times     = string(WTHB_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = WTHB_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(WTHB_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(WTHB_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = WTHB_PreBack (:,3,i);
        export_PreB_CO2         = WTHB_PreBack (:,7,i);
        export_PreB_iCH4        = WTHB_PreBack (:,5,i);
        export_PreB_iCO2        = WTHB_PreBack (:,8,i);
        export_PostB_CH4        = WTHB_PostBack(:,3,i);
        export_PostB_CO2        = WTHB_PostBack(:,7,i);
        export_PostB_iCH4       = WTHB_PostBack(:,5,i);
        export_PostB_iCO2       = WTHB_PostBack(:,8,i);
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            'WTHB_PreBack_DateTimes' , 'WTHB_PreBack' , ...
                            'WTHB_ChamON_DateTimes'  , 'WTHB_ChamON'  , ...
                            'WTHB_PostBack_DateTimes', 'WTHB_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
