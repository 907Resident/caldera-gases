...
%% PRCN_20Jun2018_Data Analysis

% PRCN_20Jun2018_DataAnalysis
% Geoscientists: Ajayi, M. & Mans, W.
% Location: (North) Porcelain Basin(PRCN), Yellowstone National Park, WY
% Data Type: Transect Measurements 
% Metadata File: To be created (23 July 2018)
...
data_dir = fullfile(fileparts(fileparts(working_dir)),                  ...
    "non-pertinent-data/dat-files/yellowstone/20Jun2018");
...
%% Gather the PRCN_20Jun2018 Data from the Forerunner Matrix 'PD_mtrx'
...
% Calculate the relative time for each measurement 
    ...
    TT_PicData    = addvars(TT_PicData,PRCN_rel_sec,                    ...
                           'Before','HP_CH4_dry');
...
% the 0's will indicate rows that are to be removed.
    % For the PRCN_20Jun2018, the data presented groups of four thus, only
...
RN(i) = 1;
    ...
    % measurement vectors
RN_CH4 = [RN, datenum(TT_PicData.Time), PRCN_rel_sec                    ...
...
% Reassign the variables so that they represent the filtered information
    ...
    % *TT_PRCN*
    TT_PRCN       = timetable(Time, RN_CH4(:,03),  RN_CH4(:,04),        ...
    ...
    smry_PRCN_20Jun2018 = summary(TT_PRCN);
    % Clear extraneous variables
...
%------Figure 1A-------%
subplot(1,2,1)
    plot(TT_PRCN.Time, TT_PRCN.HP_CH4_dry, 'o')
        title('PRCN 20Jun2018 [CH_4] vs. Timestamp (ALL)')
        ylabel('[CH_4] (ppm)')
        grid on
%------Figure 1B-------%
subplot(1,2,2)
    plot(TT_PRCN.Time, TT_PRCN.CO2_dry, 'mo')
        title('PRCN 20Jun2018 [CO_2] vs. Timestamp (ALL)')
        ylabel('[CO_2] (ppm)')
        grid on
...
yyaxis left
% --- CH4 --- %
   h1 = plot(TT_PRCN.Time,TT_PRCN.HP_CH4_dry, 'o');
                    title('PRCN 20Jun2018 [CH_4] and [CO_2] vs. Time')
                    grid on
...
yyaxis right                    
% --- CO2 --- %
   h2 = plot(TT_PRCN.Time,TT_PRCN.CO2_dry, 'o');
                    h2.Marker               = 'o';
                    h2.LineStyle            = 'none';
                    h2.Color                = [1 0 1];
                    ylabel('[CO_2] (ppm)')
...
%----Timestamps have been reevaluated [08Mar20121]
    % Pre-allocate the datetime array
PreBack_Times                   = NaT([nchams, 2]);

PreBack_Times(:,1)              = datetime(                             ...
['11:43:01'; '12:18:43'; '12:53:59'; '13:29:23'; '14:05:34'; '14:39:35';...
 '16:06:36'; '16:32:43'],                                               ...
...
['11:49:19'; '12:23:37'; '12:58:09'; '13:33:57'; '14:08:58'; '14:43:06';...
 '16:12:39'; '16:36:10'],                                               ...
 'InputFormat', 'HH:mm:ss');
...
['11:49:22'; '12:23:40'; '12:58:12'; '13:34:01'; '14:09:02'; '14:43:10';...
 '16:12:43'; '16:36:13'],                                               ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
['12:10:09'; '12:43:52'; '13:18:09'; '13:54:15'; '14:29:07'; '15:03:14';...
 '16:16:51'; '16:56:49'],                                               ...
 'InputFormat', 'HH:mm:ss');
    % Extract the time from the combined date-time value
...
PostBack_Times(:,1)             = ChamON_Times(:,2)    + (30*seconds);
PostBack_Times(:,2)             = PostBack_Times(:,1)  + (3.5*minutes);
...
%% Extract the individual measurement
    ...
    PRCN_PreBack    = NaN([115, width(TT_PRCN)+1, nchams]); 
    PRCN_ChamON     = NaN([400, width(TT_PRCN)+1, nchams]);
    PRCN_PostBack   = NaN([115, width(TT_PRCN)+1, nchams]);
...
%% Extract  datetimes
    ...
    % PreBack
        % Pre-allocate
    PRCN_PreBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(PRCN_PreBack(:,1,idx)));
    PRCN_PreBack_DateTimes(1:min(f)-1,1,idx)    = datetime(             ...
         datestr(PRCN_PreBack(1:min(f)-1,1,idx)));
end

    % ChamON
        % Pre-allocate
    PRCN_ChamON_DateTimes           = NaT([400 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(PRCN_ChamON(:,1,idx)));
    PRCN_ChamON_DateTimes(1:min(f)-1,1,idx)    = datetime(              ...
         datestr(PRCN_ChamON(1:min(f)-1,1,idx)));
end

    % PostBack
        % Pre-allocate
    PRCN_PostBack_DateTimes          = NaT([100 1 nchams]);
for idx = 1:nchams
    f                               = find(isnan(PRCN_PostBack(:,1,idx)));
    PRCN_PostBack_DateTimes(1:min(f)-1,1,idx)    = datetime(            ...
         datestr(PRCN_PostBack(1:min(f)-1,1,idx)));
end
...
[fig03, fig04, fig05] = basic_gas_viz(PRCN_ChamON, nchams, site_tag,    ...
...
PRCN_ChamON = gas_unit_prep(PRCN_ChamON, metadata.site_temperature_C, metadata.pressure_pa);
...
[x_exp_prepData, y_exp_prepData]    =                                   ...
                 prepareCurveData(PRCN_ChamON(:,02,i),                  ...
                                  PRCN_ChamON(:,10,i));   
...
[xData, yData]  = prepareCurveData((1 ./ PRCN_ChamON(:,3,idx)),           ...
                                         PRCN_ChamON(:,5,idx));
...
%--CO2--%
[xData, yData]  = prepareCurveData((1 ./ PRCN_ChamON(:,7,idx)),         ...
                                         PRCN_ChamON(:,8,idx));
    % Set up fittype and options.
KP_ft                           = fittype('poly1');
...
[last10_d13CH4,  last10_d13CO2, fig11] = last10_iso(PRCN_ChamON, "y");
...
%% Carbon Isotope Comparisons
    ...
    iCH4 = PRCN_ChamON(:,5,:); iCO2 = PRCN_ChamON(:,8,:);
...
PRCN_ChamON_Ratio           = PRCN_ChamON(:,7,:) ./ PRCN_ChamON(:,3,:);
...
PRCN_ChamON_Ratio_stats     = [mean(PRCN_ChamON_Ratio);                 ...
                               median(PRCN_ChamON_Ratio);               ...
                               range(PRCN_ChamON_Ratio);                ...
                               std(PRCN_ChamON_Ratio)];
...
for idx = 1:nchams
    ...
    nexttile
        s_CH4 = scatter(PRCN_ChamON(:,2,idx), PRCN_ChamON_Ratio(:,idx), ...
...
for i = 1:nchams
     ...
     X = PRCN_ChamON(:,5,i); Y = PRCN_ChamON(:,8,i); 
     % Color the data according to the CO2-CH4 ratio
     color = PRCN_ChamON_Ratio(:,i);
     ...
     c = colorbar;
       ...
       %c.Limits       = [min(PRCN_ChamON_Ratio) max(PRCN_ChamON_Ratio)];
...
% (y-axis), [CO2]-[CH4] ratio (z-axis)
    ...
    xyz = NaN([length(PRCN_ChamON), 3, nchams]);
    for i = 1:nchams
        xyz(:,1,i) = PRCN_ChamON(:,5,i);
        xyz(:,2,i) = PRCN_ChamON(:,8,i);
        xyz(:,3,i) = PRCN_ChamON_Ratio(:,1,i);
    end
...
PRCN_PreBack(PRCN_PreBack == 0)     = NaN;
PRCN_PostBack(PRCN_PostBack == 0)   = NaN;

for i = 1:nchams 
    % Descripitive Statistics
        % CH4
    background_CH4{i,1}          = desc_stats(PRCN_PreBack(:,3,i));
    background_CH4{i,2}          = desc_stats(PRCN_PostBack(:,3,i));
    background_CH4{i,3}          = desc_stats(PRCN_PreBack(:,5,i));
    background_CH4{i,4}          = desc_stats(PRCN_PostBack(:,5,i));
         % CO2
    background_CO2{i,1}          = desc_stats(PRCN_PreBack(:,7,i));
    background_CO2{i,2}          = desc_stats(PRCN_PostBack(:,8,i));
    background_CO2{i,3}          = desc_stats(PRCN_PreBack(:,7,i));
    background_CO2{i,4}          = desc_stats(PRCN_PostBack(:,8,i));   
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(PRCN_PreBack(:,3,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(PRCN_PostBack(:,3,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(PRCN_PreBack(:,7,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(PRCN_PostBack(:,7,i));
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(PRCN_PreBack(:,5,i));
    plot(xi, f)
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(PRCN_PostBack(:,5,i));
    plot(xi, f, '--', 'Color', "#80bfff")
    ...
    nexttile
        % Pre-Background
    [f, xi, bw_pre] = ksdensity(PRCN_PreBack(:,8,i));
    plot(xi, f, 'm')
    hold on
        % Post-Background
    [f, xi, bw_post] = ksdensity(PRCN_PostBack(:,8,i));
    plot(xi, f, '--', 'Color', "#cc33ff")
...
if writeXL == 1
    ...
    for i = 1:nchams
        ...
        export_ChamON_times     = string(PRCN_ChamON_DateTimes(:,1,i));
        % Export the end of the flux measurement time
        export_flx_end_time     = flx_end_time;
        % Emport Chamber Data
        export_chamON_data      = PRCN_ChamON(:,2:end,i);
        ...
        export_PreB_Times       = string(PRCN_PreBack_DateTimes(:,1,i));
        export_PostB_Times      = string(PRCN_PostBack_DateTimes(:,1,i));
        % Export Pre- and Post-background gas data
        export_PreB_CH4         = PRCN_PreBack (:,3,i);
        export_PreB_CO2         = PRCN_PreBack (:,7,i);
        export_PreB_iCH4        = PRCN_PreBack (:,5,i);
        export_PreB_iCO2        = PRCN_PreBack (:,8,i);
        export_PostB_CH4        = PRCN_PostBack(:,3,i);
        export_PostB_CO2        = PRCN_PostBack(:,7,i);
        export_PostB_iCH4       = PRCN_PostBack(:,5,i);
        export_PostB_iCO2       = PRCN_PostBack(:,8,i);
        ...
        msg_str = sprintf('Tab ''PRCN_%d'' has been created', i);
        disp(msg_str)
    end
...
for i = 1:nchams
    ...
    save(filename,                                                      ...
                            'PRCN_PreBack_DateTimes' , 'PRCN_PreBack' , ...
                            'PRCN_ChamON_DateTimes'  , 'PRCN_ChamON'  , ...
                            'PRCN_PostBack_DateTimes', 'PRCN_PostBack', ...
                            'lin_flux'               , 'lin_slope'    , ...
                            'lin_mdl'                ,                  ...
